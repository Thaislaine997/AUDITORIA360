    ...
    try:
        ...
        if config and config.get("GCP_SERVICE_ACCOUNT_FILE"):
            ...
            if os.path.exists(credentials_path):
                logger.info(
                    f"Tentando inicializar BigQuery Client com arquivo de conta de serviço: {credentials_path}"
                )
                ...
                logger.error(
                    f"Arquivo de conta de serviço especificado não encontrado: {credentials_path}"
                )
                return None
        ...
        if project_id_from_config:
            ...
            logger.info(
                f"Usando GCP_PROJECT_ID: {project_id_from_config} para o cliente BigQuery."
            )
        else:
            logger.info(
                "Tentando inicializar BigQuery Client com credenciais padrão do ambiente e projeto inferido."
            )
        ...
        if client.project:
            logger.info(
                f"BigQuery Client inicializado com sucesso. Projeto: {client.project}"
            )
        else:
            logger.warning(
                "BigQuery Client inicializado, mas o projeto não foi determinado. "
        ...
        logger.error(
            f"Erro específico do Google Cloud ao inicializar cliente BigQuery: {e}",
            exc_info=True,
        )
        return None
    except Exception as e:
        logger.error(
            f"Erro genérico ao inicializar cliente BigQuery: {e}", exc_info=True
        )
        return None
...
class ControleFolhaLoader:
    def __init__(self, config: Dict[str, Any]):
        ...
        """
        self.project_id: str
        self.dataset_id: str = cast(
            str, config.get("CONTROL_BQ_DATASET_ID")
        )  # Cast para informar ao linter
        ...
        if not self.client:
            raise ConnectionError(
                "Falha ao inicializar o cliente BigQuery para ControleFolhaLoader."
            )
        ...
        if project_id_from_config:
            ...
            if not self.client.project:
                logger.info(
                    f"Reconfigurando cliente BigQuery com projeto explícito: {self.project_id} usando a configuração original."
                )
                ...
                if not reconfigured_client:
                    logger.error(
                        f"Falha ao reconfigurar cliente com projeto {self.project_id} usando get_bigquery_client."
                    )
                    raise ConnectionError(
                        f"Falha ao reconfigurar cliente com projeto {self.project_id}"
                    )
                self.client = reconfigured_client
        elif self.client.project:  # Se a config não tinha, mas o cliente inferiu
            self.project_id = self.client.project
            logger.info(
                f"Usando projeto inferido pelo cliente BigQuery: {self.project_id}"
            )
        else:  # Nem config, nem cliente inferiu
            raise ValueError(
                "GCP_PROJECT_ID é obrigatório na configuração ou deve ser inferível pelas credenciais do ambiente."
            )
        ...
        if not self.client.project:  # Checagem final
            raise ConnectionError(
                f"Cliente BigQuery não tem projeto associado após tentativas de configuração. Projeto necessário: {self.project_id}"
            )
    ...
    def _get_controle_folha_schema(self) -> List[bigquery.SchemaField]:
        return [
            bigquery.SchemaField(
                ...
                "STRING",
                mode="REQUIRED",
                description="ID único da folha de pagamento, preferencialmente UUID",
            ),
            bigquery.SchemaField(
                "id_cliente",
                "STRING",
                mode="REQUIRED",
                description="ID do cliente/empresa principal dona dos dados",
            ),
            bigquery.SchemaField(
                "codigo_empresa_folha",
                "STRING",
                mode="NULLABLE",
                description="Código interno da empresa/filial na folha de pagamento de origem",
            ),
            bigquery.SchemaField(
                "cnpj_empresa_folha",
                "STRING",
                mode="NULLABLE",
                description="CNPJ da empresa/filial na folha de pagamento de origem",
            ),
            bigquery.SchemaField(
                "competencia_folha",
                "DATE",
                mode="REQUIRED",
                description="Mês e ano de competência da folha (primeiro dia do mês, ex: YYYY-MM-01)",
            ),
            bigquery.SchemaField(
                "tipo_folha",
                "STRING",
                mode="REQUIRED",
                description="Tipo da folha (ex: MENSAL, ADIANTAMENTO, 13_SALARIO_1A_PARCELA, 13_SALARIO_2A_PARCELA, PLR, FERIAS, RESCISAO)",
            ),
            bigquery.SchemaField(
                "status_processamento",
                "STRING",
                mode="REQUIRED",
                description="Status do processamento (PENDENTE, RECEBIDO, VALIDANDO, PROCESSANDO, CONCLUIDO_SUCESSO, CONCLUIDO_ERROS, ERRO_FATAL)",
            ),
            bigquery.SchemaField(
                "data_upload_cliente",
                "TIMESTAMP",
                mode="REQUIRED",
                description="Data e hora UTC do upload do arquivo pelo cliente/sistema",
            ),
            bigquery.SchemaField(
                "data_inicio_processamento",
                "TIMESTAMP",
                mode="NULLABLE",
                description="Data e hora UTC do início do processamento da folha",
            ),
            bigquery.SchemaField(
                "data_fim_processamento",
                "TIMESTAMP",
                mode="NULLABLE",
                description="Data e hora UTC da conclusão ou erro final do processamento",
            ),
            bigquery.SchemaField(
                "nome_arquivo_original",
                "STRING",
                mode="NULLABLE",
                description="Nome do arquivo original enviado pelo cliente",
            ),
            bigquery.SchemaField(
                "hash_arquivo",
                "STRING",
                mode="NULLABLE",
                description="Hash (ex: SHA256) do arquivo original para detecção de duplicatas e integridade",
            ),
            bigquery.SchemaField(
                "caminho_gcs_arquivo_original",
                "STRING",
                mode="NULLABLE",
                description="Caminho no Google Cloud Storage do arquivo original",
            ),
            bigquery.SchemaField(
                "caminho_gcs_arquivo_processado",
                "STRING",
                mode="NULLABLE",
                description="Caminho no GCS do arquivo após conversão/limpeza inicial (se aplicável)",
            ),
            bigquery.SchemaField(
                "numero_linhas_arquivo",
                "INTEGER",
                mode="NULLABLE",
                description="Número de linhas no arquivo original (informativo)",
            ),
            bigquery.SchemaField(
                "numero_colaboradores_unicos",
                "INTEGER",
                mode="NULLABLE",
                description="Número de colaboradores únicos identificados na folha processada",
            ),
            bigquery.SchemaField(
                "total_proventos",
                "NUMERIC",
                mode="NULLABLE",
                description="Soma total de proventos da folha processada",
            ),
            bigquery.SchemaField(
                "total_descontos",
                "NUMERIC",
                mode="NULLABLE",
                description="Soma total de descontos da folha processada",
            ),
            bigquery.SchemaField(
                "total_liquido",
                "NUMERIC",
                mode="NULLABLE",
                description="Soma total líquido da folha processada",
            ),
            bigquery.SchemaField(
                "data_vencimento_guia_fgts",
                "DATE",
                mode="NULLABLE",
                description="Data de vencimento da guia FGTS (se aplicável e extraída)",
            ),
            bigquery.SchemaField(
                "data_vencimento_darf_inss",
                "DATE",
                mode="NULLABLE",
                description="Data de vencimento do DARF INSS (se aplicável e extraída)",
            ),
            bigquery.SchemaField(
                "data_vencimento_darf_irrf",
                "DATE",
                mode="NULLABLE",
                description="Data de vencimento do DARF IRRF (se aplicável e extraída)",
            ),
            bigquery.SchemaField(
                "observacoes_processamento",
                "STRING",
                mode="NULLABLE",
                description="Observações ou logs resumidos do processo de importação/validação",
            ),
            bigquery.SchemaField(
                "detalhes_erros_processamento",
                "STRING",
                mode="NULLABLE",
                description="Detalhes de erros ocorridos durante o processamento (pode ser JSON stringified)",
            ),
            bigquery.SchemaField(
                "usuario_responsavel_upload",
                "STRING",
                mode="NULLABLE",
                description="Identificador do usuário ou sistema que realizou o upload",
            ),
            bigquery.SchemaField(
                "id_job_processamento",
                "STRING",
                mode="NULLABLE",
                description="ID do job ou tarefa que processou esta folha (para rastreabilidade)",
            ),
            bigquery.SchemaField(
                "data_criacao_registro",
                "TIMESTAMP",
                mode="REQUIRED",
                description="Data e hora UTC da criação deste registro na tabela de controle",
            ),
            bigquery.SchemaField(
                "data_ultima_atualizacao_registro",
                "TIMESTAMP",
                mode="REQUIRED",
                description="Data e hora UTC da última atualização deste registro",
            ),
        ]
    ...
    def _ensure_controle_table_exists(self):
        if not self.client or not self.project_id or not self.dataset_id:
            raise ConnectionError(
                "Cliente BigQuery, project_id ou dataset_id não configurados corretamente."
            )

        ensure_dataset_exists(self.dataset_id, self.project_id, self.client)
        ensure_table_exists_or_updated(
            self.full_table_id, self._get_controle_folha_schema(), self.client
        )

    def inserir_folha(
        self, dados_folha: Dict[str, Any]
        ...
        logger.debug(
            f"Tentando inserir linha na tabela {self.full_table_id}: {row_to_insert}"
        )
        try:
            errors_bq: Sequence[Dict[str, Any]] = self.client.insert_rows_json(
                self.full_table_id, [row_to_insert]
            )
            if not errors_bq:
                logger.info(
                    f"Registro de folha {row_to_insert['id_folha']} inserido com sucesso em {self.full_table_id}."
                )
                return None
            else:
                ...
                logger.error(
                    f"Erro ao inserir registro de folha em {self.full_table_id}: {error_list}"
                )
                return error_list
        except google_exceptions.GoogleCloudError as e:
            logger.error(
                f"Erro do Google Cloud ao inserir dados na tabela {self.full_table_id}: {e}",
                exc_info=True,
            )
            return [{"message": f"Erro do Google Cloud: {str(e)}"}]
        except Exception as e:
            logger.error(
                f"Erro inesperado ao inserir dados na tabela {self.full_table_id}: {e}",
                exc_info=True,
            )
            return [{"message": f"Erro inesperado: {str(e)}"}]

    def atualizar_status_folha(
        self,
        id_folha: str,
        novo_status: str,
        ...
        set_clauses = [
            ...
            "data_ultima_atualizacao_registro = @timestamp_atualizacao",
        ]
        query_params = [
            ...
            bigquery.ScalarQueryParameter(
                "timestamp_atualizacao", "TIMESTAMP", now_utc_iso
            ),
        ]
        ...
        try:
            ...
            query_job.result()
            if query_job.errors:
                error_list_update: List[Dict[str, Any]] = list(
                    cast(Sequence[Dict[str, Any]], query_job.errors)
                )
                logger.error(
                    f"Erro ao executar query de atualização para folha {id_folha}: {error_list_update}"
                )
                return error_list_update
            if (
                query_job.num_dml_affected_rows is not None
                and query_job.num_dml_affected_rows > 0
            ):
                logger.info(
                    f"Status da folha {id_folha} atualizado. Linhas afetadas: {query_job.num_dml_affected_rows}"
                )
                return None
            else:
                logger.warning(
                    f"Nenhuma linha atualizada para folha {id_folha}. A folha pode não existir."
                )
                return [{"message": f"Nenhuma linha afetada para id_folha {id_folha}."}]
        except google_exceptions.GoogleCloudError as e:
            logger.error(
                f"Erro Google Cloud ao atualizar folha {id_folha}: {e}", exc_info=True
            )
            return [{"message": f"Erro Google Cloud: {str(e)}"}]
        except Exception as e:
            logger.error(
                f"Erro inesperado ao atualizar folha {id_folha}: {e}", exc_info=True
            )
            return [{"message": f"Erro inesperado: {str(e)}"}]
            ...
            [
                {
                    "empresa_id": 1,
                    "nome": "Empresa A",
                    "client_id": getattr(self, "client_id", "simulado"),
                },
                {
                    "empresa_id": 2,
                    "nome": "Empresa B",
    ...
    def client_id(self):
        ...
        return (
            ...
            or self.__dict__.get("client_id", None)
            or self.__dict__.get("client_id", "simulado")
        )
...
def load_data_to_bq(
    ...
    try:
        ...
        load_job.result()
        if load_job.errors:
            error_list_load: List[Dict[str, Any]] = list(
                cast(Sequence[Dict[str, Any]], load_job.errors)
            )
            logger.error(
                f"Erro durante o carregamento para {full_table_id}: {error_list_load}"
            )
            return error_list_load
        else:
            logger.info(
                f"Dados carregados para {full_table_id}. Linhas: {load_job.output_rows}"
            )
            return None
    except google_exceptions.GoogleCloudError as e:
        logger.error(
            f"Erro Google Cloud ao carregar para {full_table_id}: {e}", exc_info=True
        )
        return [{"message": f"Erro Google Cloud: {str(e)}"}]
    except Exception as e:
        logger.error(
            f"Erro inesperado ao carregar para {full_table_id}: {e}", exc_info=True
        )
        return [{"message": f"Erro inesperado: {str(e)}"}]


def ensure_dataset_exists(
    dataset_id: str, project_id: str, client: bigquery.Client, location: str = "US"
    ...
    try:
        ...
        logger.info(
            f"Dataset {full_dataset_id} não encontrado. Criando em {location}..."
        )
        ...
        try:
            ...
            logger.error(
                f"Falha ao criar dataset {full_dataset_id}: {e}", exc_info=True
            )
            return None
        except Exception as e:
            logger.error(
                f"Erro genérico ao criar dataset {full_dataset_id}: {e}", exc_info=True
            )
            return None
    except google_exceptions.GoogleCloudError as e:
        logger.error(
            f"Erro Google Cloud ao verificar dataset {full_dataset_id}: {e}",
            exc_info=True,
        )
        return None
    except Exception as e:
        logger.error(
            f"Erro desconhecido ao verificar dataset {full_dataset_id}: {e}",
            exc_info=True,
        )
        return None
...
def ensure_table_exists_or_updated(
    ...
    if not client:
        logger.error(
            "Cliente BigQuery não fornecido para ensure_table_exists_or_updated."
        )
        return None
    ...
    try:
        ...
        if existing_table.time_partitioning != time_partitioning:
            logger.info(
                f"Configuração de TimePartitioning da tabela {full_table_id} difere."
            )
            needs_update = True
        # Removido RangePartitioning: não suportado nesta versão
        if set(existing_table.clustering_fields or []) != set(clustering_fields or []):
            logger.info(
                f"Configuração de ClusteringFields da tabela {full_table_id} difere."
            )
            needs_update = True
        ...
        if needs_update:
            ...
            fields_to_update = [
                "schema"
            ]  # Sempre tenta atualizar o schema se for diferente
            if (
                time_partitioning is not None
                or existing_table.time_partitioning is not None
            ):  # Se um deles está definido
                fields_to_update.append("time_partitioning")
            if (
                clustering_fields is not None
                or existing_table.clustering_fields is not None
            ):
        ...
        try:
            ...
            logger.error(f"Falha ao criar tabela {full_table_id}: {e}", exc_info=True)
            return None
        except Exception as e:
            logger.error(
                f"Erro genérico ao criar tabela {full_table_id}: {e}", exc_info=True
            )
            return None
    except google_exceptions.GoogleCloudError as e:
        logger.error(
            f"Erro Google Cloud ao acessar/atualizar {full_table_id}: {e}",
            exc_info=True,
        )
        return None
    except Exception as e:
        logger.error(
            f"Erro desconhecido ao acessar/atualizar {full_table_id}: {e}",
            exc_info=True,
        )
        return None
