...
def validate_configuration():
    ...
    if os.path.exists('.env.template'):
        ...
        required_vars = ['OPENAI_API_KEY', 'OPENAI_MODEL', 'OPENAI_MAX_TOKENS', 'OPENAI_TEMPERATURE']
        template_has_all = all(var in template_content for var in required_vars)
        checks.append(("✅" if template_has_all else "❌", ".env.template has all required variables"))
    else:
    ...
    if os.path.exists('requirements.txt'):
        ...
        checks.append(("✅" if has_openai else "❌", "OpenAI dependency in requirements.txt"))
        checks.append(("✅" if has_dotenv else "❌", "python-dotenv dependency in requirements.txt"))
    else:
    ...
    if os.path.exists('.gitignore'):
        ...
        checks.append(("✅" if env_ignored else "❌", ".env properly ignored in .gitignore"))
    else:
...
def validate_code_structure():
    ...
    if os.path.exists(openai_service_path):
        ...
        checks.append(("✅" if has_openai_class else "❌", "OpenAIService class defined"))
        checks.append(("✅" if has_chat_method else "❌", "Chat completion method exists"))
        checks.append(("✅" if has_doc_analysis else "❌", "Document analysis method exists"))
    else:
    ...
    if os.path.exists(ai_agent_path):
        ...
        checks.append(("✅" if has_openai_integration else "❌", "AI agent has OpenAI integration"))
    ...
    if os.path.exists(api_router_path):
        ...
        has_chat_endpoint = '/chat' in router_content and 'chat_with_bot' in router_content
        has_analyze_endpoint = 'analyze_document_with_ai' in router_content
        has_status_endpoint = '/status' in router_content
        
        checks.append(("✅" if has_chat_endpoint else "❌", "Chat endpoint implemented"))
        checks.append(("✅" if has_analyze_endpoint else "❌", "Document analysis endpoint implemented"))
        checks.append(("✅" if has_status_endpoint else "❌", "Status endpoint implemented"))
    else:
