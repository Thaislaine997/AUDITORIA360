...
class ProductionDeployer:
    ...
    def deploy_to_vercel(self):
        ...
        if deploy_script.exists():
            ...
            self.log_step("⚠️  Script deploy_vercel.sh não encontrado, criando configuração básica", "warning")
    ...
    def setup_production_monitoring(self):
        ...
        if monitoring_script.exists():
            try:
                result = subprocess.run([sys.executable, str(monitoring_script)], 
                                      cwd=self.project_root, capture_output=True, text=True)
                if result.returncode == 0:
                    self.log_step("✅ Monitoramento configurado", "success")
                    return True
                else:
                    self.log_step("⚠️  Configuração de monitoramento com warnings", "warning")
                    return True
            except Exception as e:
