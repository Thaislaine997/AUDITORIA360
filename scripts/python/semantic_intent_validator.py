...
class SemanticIntentAnalyzer(ast.NodeVisitor):
    ...
    def _analyze_payroll_function(self, node: ast.FunctionDef):
        ...
        if any(
            ...
            for keyword in ["vacation", "ferias", "holiday"]
        ):
                    ...
                    if self._could_return_negative(child.value):
                        self.violations.append(
                            SemanticViolation(
                                ...
                                severity="critical",
                                business_rule=self.business_rules["payroll"][
        ...
        if any(
            keyword in node.name.lower()
            for keyword in ["salary", "salario", "calculate", "calcular"]
        ):
            ...
            if (
                not has_validation
                and len([n for n in ast.walk(node) if isinstance(n, ast.Return)]) > 0
            ):
                self.violations.append(
                    SemanticViolation(
                        file=self.file_path,
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Call):
                ...
                if func_name and any(
                    ...
                    for log_func in ["log", "print", "debug"]
                ):
                    ...
                    if self._contains_sensitive_data_reference(child):
                        self.violations.append(
                            SemanticViolation(
                                ...
                                severity="critical",
                                business_rule=self.business_rules["security"][
                                    "data_exposure"
                                ],
                            )
                        )
...
def main():
    ...
    if all_violations:
        ...
        print(
            "✅ No semantic violations detected. Code aligns with its intended purpose."
        )
