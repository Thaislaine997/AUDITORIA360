...
class VercelDeployer:
    ...
    def __init__(
        self, production: bool = False, dry_run: bool = False, verbose: bool = False
    ):
    ...
    def validate_prerequisites(self) -> None:
        ...
        if (
            not (self.project_root / "package.json").exists()
            and not (self.project_root / "vercel.json").exists()
        ):
    ...
    def validate_project_config(self) -> None:
        ...
        if vercel_config.exists():
            ...
            logger.warning(
                "Arquivo vercel.json não encontrado - usando configuração padrão"
            )
            ...
            try:
                with open(package_json) as f:
                    ...
                    if "build" in package_data.get("scripts", {}):
                        logger.info(
                            "Script de build encontrado - executando build local..."
                        )
    ...
    def _get_vercel_user(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["vercel", "whoami"], capture_output=True, text=True, check=True
            )
...
Examples:
  ...
  %(prog)s --verbose             Enable verbose output
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simulate execution without deploying"
    )
    parser.add_argument(
    ...
    try:
        ...
        deployer = VercelDeployer(
            production=args.production, dry_run=args.dry_run, verbose=args.verbose
        )
