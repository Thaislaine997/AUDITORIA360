...
class BusinessLogicAnalyzer(ast.NodeVisitor):
    ...
    def _validate_payroll_function(self, node: ast.FunctionDef):
        ...
        if any(
            keyword in function_name for keyword in ["vacation", "ferias", "holiday"]
        ):
        ...
        if any(
            ...
            for keyword in ["salary", "salario", "calculate_pay"]
        ):
            self._check_salary_logic(node)

        # Check deduction functions
        if any(
            keyword in function_name for keyword in ["deduction", "desconto", "deduct"]
        ):
    ...
    def _validate_security_function(self, node: ast.FunctionDef):
        ...
        if any(
            ...
            for keyword in ["access", "authorize", "permission"]
        ):
