...
async def test_basic_mcp_functionality():
    ...
    try:
        ...
        print(f"✅ Server created with {len(server._tools)} tools and {len(server._resources)} resources")
        ...
        print(f"✅ Payroll calculation successful: Net salary = {result.get('net_salary')}")
        ...
        kb_content = await resource_provider._read_knowledge_base("auditoria://knowledge/base")
        ...
        print(f"✅ Configuration loaded: {len(config.servers)} servers, Copilot enabled")
...
async def test_enhanced_ai_agent():
    ...
    try:
        ...
        if agent.status == "ready":
            ...
            print(f"✅ Enhanced AI agent ready: {tools_count} tools, {resources_count} resources")
...
async def main():
    ...
    if overall_success:
        ...
        print("\n📚 Next steps:")
        print("1. Run the full development environment: ./scripts/start_dev_environment.sh")
