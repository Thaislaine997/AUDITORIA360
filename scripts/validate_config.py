        ...
        try:
            with open(".env.production", "r") as f:
                ...
                for var in critical_vars:
                    if var in content:
                        print_status(
                            f"Variável {var.split('=')[0]} configurada", "success"
                        )
                    else:
                        print_status(
                            f"Variável {var.split('=')[0]} não encontrada", "warning"
                        )
        except Exception as e:
...
def validate_deployment_scripts():
    ...
    if deploy_script.exists():
        if os.access(deploy_script, os.X_OK):
            ...
            print_status(
                "Script de deploy não é executável (execute: chmod +x deploy_streamlit.sh)",
                "warning",
            )
...
def main():
    ...
    if all_checks_passed:
        print_status(
            "🎉 Todas as verificações passaram! Pronto para deploy.", "success"
        )
        print()
