...
class ProductionValidator:
    ...
    def log(self, message: str, level: str = "info"):
        ...
        print(
            f"{colors.get(level, '')}{prefix.get(level, '')} {message}{colors['reset']}"
        )
    ...
    def validate_environment_config(self) -> bool:
        ...
        for var in required_env_vars:
            if var not in env_content:
                ...
                placeholder in env_content
                for placeholder in ["your_", "change-this", "example"]
            ):
        ...
        if placeholder_vars:
            ...
            self.log(
                "🔧 Update these with real production values before deploy", "warning"
            )
            ...
            for i, line in enumerate(lines, 1):
                ...
                if line and not line.startswith("#"):
                    ...
                    if not any(
                        ...
                        self.log(
                            f"⚠️ Potentially invalid requirement at line {i}: {line}",
                            "warning",
                        )

            self.log(
                f"✅ Requirements file validated ({len([l for l in lines if l.strip() and not l.startswith('#')])} packages)",
                "success",
            )
            return True
    ...
    def validate_vercel_config(self) -> bool:
        ...
        try:
            ...
            if "builds" in config:
                api_build = any(
                    "api/index.py" in str(build.get("src", ""))
    ...
    def validate_streamlit_config(self) -> bool:
        ...
        secrets_template_path = (
            self.project_root / ".streamlit" / "secrets.toml.template"
        )
    ...
    def validate_security_measures(self) -> bool:
        ...
        if missing_patterns:
            self.log(
                f"⚠️ Security patterns missing from .gitignore: {missing_patterns}",
                "warning",
            )
    ...
    def run_basic_tests(self) -> bool:
        ...
        for import_stmt in test_imports:
            success, output = self.run_command([sys.executable, "-c", import_stmt])
    ...
    def generate_validation_report(self) -> Dict[str, Any]:
        ...
        if self.warnings:
            report["recommendations"].append(
                "Address warning items before production deploy"
            )
    ...
    def run_full_validation(self) -> bool:
        ...
        if all_passed and not self.critical_issues:
            ...
            self.log(
                "⚠️ VALIDATION COMPLETED - Address warnings before deploy", "warning"
            )
...
def main():
    ...
    parser.add_argument(
        "--fix-issues", action="store_true", help="Attempt to fix found issues"
    )
