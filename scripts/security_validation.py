...
EXCLUSIONS = {
    'files': ['.git/', '__pycache__/', '.env.example', '.env.template', 'secrets.toml.template'],
...
def is_safe_pattern(text, context=""):
    ...
    if any(keyword in context.lower() for keyword in ['exemplo', 'example', 'test', 'fake', 'dummy', 'template', 'placeholder']):
        return True
        
    # Check if it's a variable name check (not actual value)
    if 'SECRET_KEY=' in text and any(keyword in context.lower() for keyword in ['checking', 'variable', 'configured', 'vars']):
        return True
