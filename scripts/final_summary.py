    ...
    if report_file.exists():
        ...
        for category, items in issues.items():
            count = (
                ...
                else len(items.keys()) if isinstance(items, dict) else 1
            )
            status = (
                "✅ RESOLVIDO"
                if category
                in ["obsolete_files", "deprecated_pydantic", "missing_ml_dependencies"]
                else "🔍 IDENTIFICADO"
            )
            print(
                f"   • {category.replace('_', ' ').title()}: {count} itens - {status}"
            )
