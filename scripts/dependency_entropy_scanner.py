...
class DependencyEntropyScanner:
    ...
    def _make_recommendation(
        ...
        if usage_count == 0:
            return (
                "remove",
                f"Unused dependency with {maintenance_cost:.1%} maintenance cost",
            )

        if entropy_score > 2.0:
            return (
                "deprecate",
                f"High entropy score ({entropy_score:.2f}) - maintenance cost exceeds value",
            )

        if entropy_score > 1.5 and usage_count < 3:
            return (
                "deprecate",
                f"Low usage ({usage_count} imports) with high maintenance overhead",
            )

        if maintenance_cost > 0.7:
            return (
                "deprecate",
                f"High maintenance cost ({maintenance_cost:.1%}) - consider alternatives",
            )

        return (
            "keep",
            f"Good balance of value ({business_value:.1%}) vs cost ({maintenance_cost:.1%})",
        )
...
def main():
    ...
    high_entropy = [
        dep
        for dep in all_analyses
        if dep.entropy_score > 1.5 or dep.recommendation in ["deprecate", "remove"]
    ]
