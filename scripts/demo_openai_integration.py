...
async def test_openai_service():
    ...
    try:
        ...
        if result["success"]:
            ...
            print(
                f"📊 Token usage: {result.get('usage', {}).get('total_tokens', 'N/A')}"
            )
        else:
...
async def test_api_endpoints():
    ...
    try:
        ...
        endpoint_names = [
            route.name for route in router.routes if hasattr(route, "name")
        ]
...
async def main():
    ...
    if openai_success and agent_success and api_success:
        ...
        print("\nNext steps:")
        print(
            "1. Start the API server: uvicorn api.index:app --reload --host 0.0.0.0 --port 8000"
        )
