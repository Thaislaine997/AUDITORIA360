...
def transformar_features(df):
    ...
    df['proporcao_descontos'] = df['total_descontos'] / df['total_proventos'].replace(0, 1)
...
def anonimizar_dados(df):
    ...
    if 'cpf_funcionario' in df.columns:
        df['cpf_funcionario_hash'] = df['cpf_funcionario'].apply(lambda x: hashlib.sha256(str(x).encode()).hexdigest())
...
def carregar_dataset_treinamento(client, df):
    ...
    job.result()
    logging.info(f"Carregado dataset de treinamento com {len(df)} registros para {TABELA_DESTINO}.")
