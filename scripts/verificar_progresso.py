...
def verificar_cobertura_testes():
    ...
    try:
        result = subprocess.run(['pytest', '--cov=src', '--cov=api', '--cov=services', '--cov-report=json'], 
                              capture_output=True, text=True, cwd='/home/runner/work/AUDITORIA360/AUDITORIA360')
...
def verificar_automacao_serverless():
    ...
    github_workflows = Path("/home/runner/work/AUDITORIA360/AUDITORIA360/.github/workflows")
    automation_files = list(Path("/home/runner/work/AUDITORIA360/AUDITORIA360/automation").glob("*.py"))
    
    workflows_count = len(list(github_workflows.glob("*.yml"))) if github_workflows.exists() else 0
...
def gerar_relatorio_progresso():
    ...
    print(f"🚀 Deploy Dashboards: {'✅ Configurado' if dashboards_ok else '❌ Pendente'}")
