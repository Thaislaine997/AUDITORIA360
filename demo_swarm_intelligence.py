...
from src.mcp.swarm import CollectiveMind, BaseAgent, SpecialistAgent, AgentRole
...
async def run_full_quantum_validation(validator):
    ...
    try:
        ...
        print(f"✅ Testes aprovados: {results['tests_passed']}/{results['total_tests']}")
        print(f"📊 Taxa de sucesso: {results['success_rate']:.1f}%")
        
        # Status detalhado
        assessment = results['final_assessment']
        print(f"\n📋 Avaliação Final:")
        print(f"   🧠 Inteligência de Enxame: {'✅ OPERACIONAL' if assessment['swarm_intelligence_operational'] else '❌ FALHOU'}")
        print(f"   🎭 Comportamento Emergente: {'✅ CONFIRMADO' if assessment['emergent_behavior_confirmed'] else '❌ NÃO DETECTADO'}")
        print(f"   🔄 Auto-Cura: {'✅ VERIFICADO' if assessment['self_healing_verified'] else '❌ FALHOU'}")
        print(f"   🦋 Adaptação Dinâmica: {'✅ PROVADO' if assessment['dynamic_adaptation_proven'] else '❌ FALHOU'}")
        print(f"   💭 Consciência Eficiente: {'✅ MEDIDO' if assessment['consciousness_efficiency_measured'] else '❌ FALHOU'}")
        ...
        # Salvar resultados
        results_file = f"/tmp/swarm_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        import json
...
async def test_emergent_behavior(validator):
    ...
    objective = input("🎯 Objetivo vago para o coletivo (Enter para usar padrão): ").strip()
    if not objective:
    ...
    if result['new_specializations']:
        print(f"   🎓 Especializações criadas: {', '.join(result['new_specializations'])}")
    ...
    print(f"   🎨 Solução criativa: {'SIM' if behaviors['creative_problem_solving'] else 'NÃO'}")
...
async def test_corrupted_agent(validator):
    ...
    print(f"\n📊 Resultado do Teste:")
    print(f"   ✅ Teste passou: {'SIM' if result['test_passed'] else 'NÃO'}")
    print(f"   🔍 Corrupção detectada: {'SIM' if result['corruption_detected'] else 'NÃO'}")
    ...
    print(f"   ❌ Proposta maliciosa rejeitada: {'SIM' if response['consensus_rejected'] else 'NÃO'}")
    print(f"   🚨 Isolamento automático: {'SIM' if response['automatic_isolation'] else 'NÃO'}")
    print(f"   🛡️ Integridade mantida: {'SIM' if response['system_integrity_maintained'] else 'NÃO'}")
...
async def test_dynamic_specialization(validator):
    ...
    print(f"\n📊 Resultado do Teste:")
    print(f"   ✅ Teste passou: {'SIM' if result['test_passed'] else 'NÃO'}")
    print(f"   👥 Novos agentes: {result['final_agent_count'] - result['initial_agent_count']}")
    print(f"   🎓 Novas especializações: {len(result['new_specializations_created'])}")
    print(f"   🎯 Treinamento detectado: {'SIM' if result['automatic_training_detected'] else 'NÃO'}")
    print(f"   ⏱️  Duração: {result['test_duration_seconds']:.1f}s")
    
    if result['new_specializations_created']:
        print(f"   📚 Especializações: {', '.join(result['new_specializations_created'])}")
    
    coverage = result['domain_coverage']
    print(f"\n🌐 Cobertura do Domínio:")
    print(f"   📦 Logística coberta: {'SIM' if coverage['logistics_covered'] else 'NÃO'}")
    print(f"   🎓 Fine-tuning simulado: {'SIM' if coverage['fine_tuning_simulated'] else 'NÃO'}")
    print(f"   ✅ Tarefa atribuída: {'SIM' if coverage['task_assignment_successful'] else 'NÃO'}")
...
async def test_consciousness_cost(validator):
    ...
    print(f"   🤖 Custo por agente: {result['collective_thinking_cost']['cost_per_agent']:.1f}")
    print(f"   📈 Rating eficiência: {result['collective_thinking_cost']['efficiency_rating']}")
...
async def show_collective_health(collective_mind):
    ...
    print(f"🚨 Protocolos de emergência: {'ATIVO' if health['emergency_protocols_active'] else 'INATIVO'}")
    ...
    print(f"   🦠 Agentes corrompidos detectados: {metrics['corrupted_agents_detected']}")
    ...
    if hasattr(health['consciousness_cost'], 'result'):
        ...
        print(f"   ⚡ Eficiência comunicação: {consciousness['communication_efficiency']:.3f}")
...
async def create_manual_specialist(trainer):
    ...
    try:
        agent_id = await trainer.train_specialist_for_domain(domain, specialization_type=specialization)
...
async def demonstrate_complex_problem(collective_mind):
    ...
    choice = input(f"\n🤔 Escolha um problema (1-{len(problems)}) ou digite um customizado: ").strip()
    ...
    print(f"   🎭 Comportamento emergente: {'SIM' if result['emergent_behavior_detected'] else 'NÃO'}")
...
async def visualize_collective_state(collective_mind):
    ...
    if not recent_messages:
        ...
        for msg in recent_messages:
            ...
            recipient = msg.recipient_id[:8] + "..." if msg.recipient_id and len(msg.recipient_id) > 8 else msg.recipient_id or "TODOS"
            print(f"   📨 {sender} → {recipient}: {msg.message_type}")
    
    print(f"\n📋 TAREFAS ATIVAS:")
    active_tasks = [task for task in collective_mind.tasks.values() if task.status in ["pending", "in_progress"]]
    if not active_tasks:
        ...
        for task in active_tasks[:3]:  # Mostrar só as primeiras 3
            ...
            if task.assigned_agents:
                agents_str = ", ".join(agent[:8] + "..." for agent in task.assigned_agents[:2])
                print(f"      👥 Agentes: {agents_str}")
    
    print(f"\n🗳️ CONSENSO ATIVO:")
    active_proposals = [p for p in collective_mind.consensus_proposals.values() if p.status == "pending"]
