...
def initialize_session_state_checklist():
    ...
    if (
        "current_folha_id_for_checklist" not in st.session_state
    ):  # Renomeado de id_folha_processada_checklist
...
def fetch_checklist_items(id_cliente: str, id_folha_processada: str):
    ...
    try:
        ...
        logger.error(
            f"Erro de conexão ao buscar itens do checklist: {e}", exc_info=True
        )
        return []
    except json.JSONDecodeError:
        st.error("Erro ao decodificar a resposta da API (itens do checklist).")
        logger.error(
            "Erro ao decodificar a resposta da API (itens do checklist).", exc_info=True
        )
        return []


def update_checklist_item_api(
    id_cliente: str, id_folha_processada: str, id_item_checklist: str, updates: dict
):
    ...
    try:
        response = requests.put(
            api_url, json=updates, headers=get_auth_headers_checklist()
        )
        if response.status_code == 401:
            handle_api_error(response.status_code)
            st.rerun()
        ...
        logger.error(
            f"Erro de conexão ao atualizar item do checklist: {e}", exc_info=True
        )
        return False
    except (
        json.JSONDecodeError
    ):  # Embora PUT não espere JSON de volta tipicamente, pode haver erro com JSON
        st.error("Erro ao processar a resposta da API (atualização do item).")
        logger.error(
            "Erro ao processar a resposta da API (atualização do item).", exc_info=True
        )
        return False
...
def mark_sheet_as_closed_api(id_cliente: str, id_folha_processada: str):
    ...
    try:
        ...
        logger.error(
            f"Erro de conexão ao marcar folha como fechada: {e}", exc_info=True
        )
        return False
...
def get_dica_ia_api(
    ...
    try:
        response = requests.post(
            api_url, json=payload, headers=get_auth_headers_checklist()
        )
        if response.status_code == 401:
            handle_api_error(response.status_code)
            st.rerun()
...
def fetch_folhas_disponiveis_para_checklist(id_cliente: str):
    ...
    try:
        ...
        for folha_data in folhas_raw:
            try:
                ...
                if isinstance(dt_str, str):
                    dt_obj = (
                        datetime.strptime(dt_str.split("T")[0], "%Y-%m-%d").date()
                        if "T" in dt_str
                ...
                logger.error(
                    f"Erro ao formatar data para folha {folha_data.get('id_folha_processada')}: {ve}"
                )
        ...
        logger.error(
            f"Erro de conexão ao buscar folhas disponíveis: {e}", exc_info=True
        )
        return []
    except json.JSONDecodeError:
        st.error("Erro ao decodificar a resposta da API (folhas disponíveis).")
        logger.error(
            "Erro ao decodificar a resposta da API (folhas disponíveis).", exc_info=True
        )
        return []
...
def mostrar_checklist_page():
    ...
    if not folhas_disponiveis:
        st.info(
            "Nenhuma folha de pagamento com checklist pendente encontrada para este cliente."
        )
        st.stop()

    map_label_to_id_folha = {
    ...
    label_folha_selecionada = st.selectbox(
        ...
        index=(
            list(map_label_to_id_folha.keys()).index(label_selecionada_default)
            if label_selecionada_default
            else 0
        ),
    ...
    if st.button("Carregar Itens do Checklist") or (
        id_folha_escolhida and not st.session_state.checklist_items
    ):
        st.session_state.checklist_items = fetch_checklist_items(
            id_cliente_atual, id_folha_escolhida
        )

    if not st.session_state.checklist_items:
        st.info(
            "Nenhum item de checklist encontrado para esta folha ou checklist ainda não carregado."
        )
    else:
        for index, item in enumerate(st.session_state.checklist_items):
            item_id = item.get(
                "id_item_checklist_fechamento"
            )  # Ajustar conforme o schema da API
            descricao = item.get("descricao_item", "Item sem descrição")
            status_atual = item.get(
            ...
            with st.container(border=True):
                ...
                with cols_status_obs[0]:
                    novo_status = st.radio(
                        ...
                        index=(
                            ["PENDENTE", "CONCLUIDO", "NAO_APLICAVEL"].index(
                                status_atual
                            )
                            if status_atual
                            in ["PENDENTE", "CONCLUIDO", "NAO_APLICAVEL"]
                            else 0
                        ),
                ...
                with cols_acao_dica[0]:
                    if st.button("Salvar Item", key=f"save_{item_id}"):
                        updates = {
                            ...
                            "observacoes_usuario": novas_observacoes,
                        }
                        if update_checklist_item_api(
                            id_cliente_atual, id_folha_escolhida, item_id, updates
                        ):
                    ...
                    if dica_cache_key not in st.session_state.dica_ia_cache:
                        if st.button("💡 Obter Dica da IA", key=f"dica_{item_id}"):
                            dica = get_dica_ia_api(
                                id_cliente_atual, id_folha_escolhida, item_id, descricao
                            )
                            if dica:
                                ...
                                st.warning(
                                    "Não foi possível obter a dica da IA no momento."
                                )
                    else:
                        st.info(
                            f"**Dica da IA (cache):** {st.session_state.dica_ia_cache[dica_cache_key]}"
                        )
        ...
        if (
            all(
                it.get("status_item") in ["CONCLUIDO", "NAO_APLICAVEL"]
                for it in st.session_state.checklist_items
            )
            and st.session_state.checklist_items
        ):
            if st.button(
                "🏁 Marcar Folha como Fechada (Checklist Concluído)", type="primary"
            ):
            ...
            st.warning(
                "Ainda existem itens pendentes no checklist. Conclua todos os itens para fechar a folha."
            )
    ...
    if tickets:
        ...
        for ticket in tickets:
            st.markdown(
                f"**{ticket['titulo']}** - {ticket['etapa']} - Responsável: {ticket['responsavel']} - Prazo: {ticket['prazo']}"
            )
    else:
...
if __name__ == "__main__":
    ...
    if "token" not in st.session_state:
        st.session_state.token = (
            "token_simulado_checklist"  # Adicione um token válido se sua API exigir
        )
    if "id_cliente" not in st.session_state:
        st.session_state.id_cliente = (
            "cliente_simulado_checklist_123"  # ID de cliente para teste
        )
    if (
        "user_info" not in st.session_state
    ):  # user_info é usado por display_user_info_sidebar
        st.session_state.user_info = {
            "name": "Usuário Teste Checklist",
            "username": "testuser_checklist",
        }
...
if token:
    ...
    checklist_filtrado = [
        c for c in checklist if filtro.lower() in c.get("item", "").lower()
    ]
