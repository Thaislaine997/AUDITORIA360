...
from src.frontend.utils.auth import obter_token, verificar_autenticacao
...
st.set_page_config(
    page_title="Trilha de Auditoria | Admin", page_icon="🔍", layout="wide"
)
...
tab_logs, tab_estatisticas, tab_seguranca = st.tabs(
    ["📋 Registros de Atividade", "📊 Estatísticas", "🔒 Segurança"]
)
...
with tab_logs:
    ...
    with col1:
        ...
        st.caption(
            f"De {formatar_timestamp(data_inicio)} até {formatar_timestamp(data_fim)}"
        )
        ...
        with st.spinner("Carregando registros de auditoria..."):
            ...
            try:
                response = requests.get(
                    f"{API_BASE}/audit-logs/", headers=headers, params=params
                )
                ...
                if response.status_code == 200:
                    ...
                    if not logs or len(logs) == 0:
                        ...
                        if "timestamp_evento" in df.columns:
                            df["timestamp_formatado"] = df["timestamp_evento"].apply(
                                formatar_timestamp
                            )

                        # Adicionar nome do cliente se disponível
                        if "id_cliente_contexto" in df.columns:
                            df["cliente"] = df["id_cliente_contexto"].apply(
                                obter_nome_cliente
                            )
                        ...
                        colunas_exibir = [
                            col for col in colunas_exibir if col in df.columns
                        ]
...
with tab_estatisticas:
    ...
    dias_estatisticas = st.slider(
        "Analisar dados dos últimos dias", 1, 90, 30, key="dias_estatisticas"
    )
                ...
                if response.status_code == 200:
                    ...
                    with col1:
                        ...
                        if "contagens" in dados:
                            ...
                            # Criar métricas
                            st.metric(
                                "Total de Eventos", contagens.get("total_geral", 0)
                            )
                        ...
                        if "por_usuario" in dados and dados["por_usuario"]:
                            ...
                            df_usuarios = pd.DataFrame(
                                [
                                    {
                                        "Usuário": u.get("email_usuario_acao", "N/A"),
                                        "Ações": u.get("total", 0),
                                    }
                                    for u in usuarios
                                ]
                            ...
                            st.dataframe(
                                df_usuarios, hide_index=True, use_container_width=True
                            )
                    ...
                    with col2:
                        ...
                        if "por_tipo" in dados and dados["por_tipo"]:
                            ...
                            df_tipo = pd.DataFrame(
                                [
                                    {
                                        "Tipo": t.get("tipo_acao", "Desconhecido"),
                                        "Quantidade": t.get("total", 0),
                                    }
                            ...
                            chart = (
                                ...
                                .properties(
                                    title="Eventos por Tipo de Ação", height=400
                                )
                            )
                    ...
                    if "por_recurso" in dados and dados["por_recurso"]:
                        ...
                        df_recursos = pd.DataFrame(
                            [
                                {
                                    "Recurso": r.get("recurso_afetado", "Desconhecido"),
                                    "Acessos": r.get("total", 0),
                                }
                                for r in recursos
                            ]
                        )
                        ...
                        chart = (
                            ...
                            .properties(
                                title="Top 10 Recursos Acessados", width=500, height=300
                            )
                        )
...
with tab_seguranca:
    ...
    dias_seguranca = st.slider(
        "Analisar alertas dos últimos dias", 1, 30, 7, key="dias_seguranca"
    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not falhas or len(falhas) == 0:
                        st.success(
                            "Nenhum alerta de segurança identificado no período analisado."
                        )
                    else:
                        st.warning(
                            f"Foram encontrados {len(falhas)} alertas de segurança potenciais."
                        )
                        ...
                        if "primeira_tentativa" in df_falhas.columns:
                            df_falhas["primeira_tentativa"] = df_falhas[
                                "primeira_tentativa"
                            ].apply(formatar_timestamp)
                        if "ultima_tentativa" in df_falhas.columns:
                            df_falhas["ultima_tentativa"] = df_falhas[
                                "ultima_tentativa"
                            ].apply(formatar_timestamp)

                        # Renomear colunas para exibição
                        df_falhas = df_falhas.rename(
                            columns={
                                "email_usuario_acao": "Usuário",
                                "ip_origem": "IP",
                    ...
                    st.error(
                        f"Erro ao obter dados de segurança: {response.status_code}"
                    )
                    if response.text:
                        st.code(response.text)
            except Exception as e:
...
config = {"api_url": "http://localhost:8000/api/trilha"}
