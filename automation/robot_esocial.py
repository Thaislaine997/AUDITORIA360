...
class EnhancedRPAESocial:
    ...
    def verify_success_checkpoint(
        self, operation: str, expected_elements: list = None
    ) -> bool:
        """
        ...
        try:
            ...
            if verification_passed:
                ...
                checkpoint_data["failure_reason"] = (
                    "Expected elements not found or error detected"
                )
            ...
            self._send_high_priority_alert(
                operation, {"error": error_msg, "traceback": traceback.format_exc()}
            )
            return False
    ...
    def _send_high_priority_alert(self, operation: str, failure_data: Dict[str, Any]):
        ...
        try:
            ...
            alert_file = (
                Path("alerts")
                / f"rpa_alert_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
def login_esocial(usuario: str, senha: str, enhanced_rpa: EnhancedRPAESocial) -> bool:
    ...
    try:
        ...
        if login_success:
            ...
            if enhanced_rpa.verify_success_checkpoint(
                "login", ["dashboard_element", "user_menu"]
            ):
                logger.info("✅ Login no eSocial realizado com sucesso")
                return True
            else:
                logger.error(
                    "❌ Login aparentemente bem-sucedido mas verificação falhou"
                )
                return False
        else:
    ...
    try:
        ...
        try:
            ...
            if submission_success:
                ...
                if enhanced_rpa.verify_success_checkpoint(
                    "event_submission", expected_elements
                ):
                    logger.info("✅ Evento enviado com sucesso ao eSocial")
                    return True
                else:
                    logger.error(
                        "❌ Envio aparentemente bem-sucedido mas verificação falhou"
                    )
        ...
        if evento_enviado:
            ...
            if enhanced_rpa.verify_success_checkpoint(
                "enviar_evento", expected_elements
            ):
                logger.info("✅ Evento enviado com sucesso ao eSocial")
                return True
            else:
...
def consultar_status(evento_id: str, enhanced_rpa: EnhancedRPAESocial) -> Optional[str]:
    ...
    try:
        ...
        if enhanced_rpa.verify_success_checkpoint(
            "consultar_status", ["status_display", "event_details"]
        ):
    ...
    try:
        ...
        if login_esocial("usuario_exemplo", "senha_exemplo", enhanced_rpa):
            ...
            if enviar_evento(evento, enhanced_rpa):
                ...
                if status:
                    logger.info(
                        f"✅ Automação eSocial concluída com sucesso. Status: {status}"
                    )

                    # Final success checkpoint
                    if enhanced_rpa.verify_success_checkpoint(
                        "executar_esocial_completo"
                    ):
                        logger.info("🎉 Processo completo verificado com sucesso!")
                    else:
                        logger.warning(
                            "⚠️ Processo concluído mas verificação final falhou"
                        )
                else:
...
def login_esocial_legacy(usuario, senha):
    ...
    logger.warning(
        "⚠️ Usando função de login legacy - considere migrar para versão aprimorada"
    )
    enhanced_rpa = EnhancedRPAESocial()
...
def enviar_evento_legacy(evento):
    ...
    logger.warning(
        "⚠️ Usando função de envio legacy - considere migrar para versão aprimorada"
    )
    enhanced_rpa = EnhancedRPAESocial()
...
def consultar_status_legacy(evento_id):
    ...
    logger.warning(
        "⚠️ Usando função de consulta legacy - considere migrar para versão aprimorada"
    )
    enhanced_rpa = EnhancedRPAESocial()
