...
class PayrollRPAServerless:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.get(
                    f"{self.api_base_url}/v1/payroll/pending", headers=headers
                )
                response.raise_for_status()
                ...
                for item in pending_items:
                    try:
                        ...
                        logger.error(
                            f"Failed to process payroll item {item.get('id')}: {e}"
                        )
    ...
    async def _process_single_payroll(
            ...
            try:
                ...
                report = {
                    ...
                    "summary": {
                        ...
                        "total_gross_pay": sum(
                            p.get("calculations", {}).get("gross_pay", 0)
                            for p in payrolls
                        ),
                        "total_net_pay": sum(
                            p.get("calculations", {}).get("net_pay", 0)
                            for p in payrolls
                        ),
                        "total_deductions": sum(
                            p.get("calculations", {}).get("deductions", 0)
                            for p in payrolls
                        ),
                    },
