...
class ScheduledReportsServerless:
    ...
    async def generate_daily_report(self) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                # Save report locally
                report_file = (
                    self.output_dir / f"daily_report_{yesterday.isoformat()}.json"
                )
            ...
            try:
                ...
                report = {
                    ...
                    "summary": {
                        ...
                        "avg_compliance_score": compliance_trends.get(
                            "average_score", 0
                        ),
                        "compliance_improvement": compliance_trends.get(
                            "improvement", 0
                        ),
                ...
                # Save report locally
                report_file = (
                    self.output_dir / f"weekly_report_{start_date}_to_{end_date}.json"
                )
                with open(report_file, "w") as f:
                    json.dump(report, f, indent=2)
    ...
    async def generate_monthly_report(self) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                tasks = [
                    client.get(
                        f"{self.api_base_url}/v1/audits/monthly",
                        headers=headers,
                        params={"month": start_date.strftime("%Y-%m")},
                    ),
                    client.get(
                        f"{self.api_base_url}/v1/compliance/monthly",
                        headers=headers,
                        params={"month": start_date.strftime("%Y-%m")},
                    ),
                    client.get(
                        f"{self.api_base_url}/v1/payroll/monthly",
                        headers=headers,
                        params={"month": start_date.strftime("%Y-%m")},
                    ),
                    client.get(
                        f"{self.api_base_url}/v1/risks/monthly",
                        headers=headers,
                        params={"month": start_date.strftime("%Y-%m")},
                    ),
                ]
                ...
                report = {
                    ...
                    "executive_summary": {
                        ...
                        "overall_compliance_score": compliance_data.get(
                            "overall_score", 0
                        ),
                    ...
                    "kpis": self._calculate_monthly_kpis(
                        audit_data, compliance_data, payroll_data, risk_data
                    ),
                    "action_items": self._generate_action_items(
                        audit_data, compliance_data, risk_data
                    ),
                }
                ...
                report_file = (
                    self.output_dir
                    / f"monthly_report_{start_date.strftime('%Y-%m')}.json"
                )
                with open(report_file, "w") as f:
                    json.dump(report, f, indent=2)
                ...
                logger.info(
                    f"Generated monthly report for {start_date.strftime('%Y-%m')}"
                )
                return report
    ...
    def _generate_recommendations(
        self, audit_summary: Dict, compliance_trends: Dict, risk_analysis: Dict
    ) -> List[Dict]:
    ...
    def _generate_action_items(
        self, audit_data: Dict, compliance_data: Dict, risk_data: Dict
    ) -> List[Dict]:
