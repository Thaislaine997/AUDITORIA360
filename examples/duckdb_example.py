...
class PayrollAnalytics:
    ...
    def _setup_sample_data(self):
        ...
        employees_data = pd.DataFrame({
            ...
            "departamento": np.random.choice(["RH", "TI", "Financeiro", "Vendas", "Marketing"], 100),
            "cargo": np.random.choice(["Analista", "Coordenador", "Gerente", "Diretor", "Estagiário"], 100),
    ...
    def tendencia_salarial(self):
        ...
        for dept in resultado["departamento"].unique():
            ...
            plt.plot(dept_data["competencia"], dept_data["salario_medio"], 
    ...
    def analise_custos(self):
        ...
        resultado["custo_por_funcionario"] = resultado["custo_total"] / resultado["funcionarios"]
        resultado["percentual_beneficios"] = (resultado["beneficios"] / resultado["custo_total"] * 100).round(1)
        resultado["percentual_encargos"] = (resultado["encargos_patronais"] / resultado["custo_total"] * 100).round(1)
    ...
    def relatorio_executivo(self):
        ...
        if len(crescimento) >= 2:
            var_funcionarios = ((crescimento.iloc[-1]["funcionarios"] - crescimento.iloc[0]["funcionarios"]) / 
                              crescimento.iloc[0]["funcionarios"] * 100)
            var_folha = ((crescimento.iloc[-1]["folha_bruta"] - crescimento.iloc[0]["folha_bruta"]) / 
                        crescimento.iloc[0]["folha_bruta"] * 100)
        else:
