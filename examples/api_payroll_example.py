...
class PayrollAPI:
    ...
    def calculate_payroll(self, calculation_data: Dict) -> Dict:
        ...
        try:
            ...
            result = response.json()
            print(
                f"✅ Folha calculada para {calculation_data.get('competency', 'N/A')}"
            )
            return result
    ...
    def import_payroll_data(self, file_path: str, import_type: str = "csv") -> Dict:
        ...
        try:
            with open(file_path, "rb") as file:
                ...
                result = response.json()
                print(
                    f"✅ Dados importados: {result.get('records_processed', 0)} registros"
                )
                return result
        ...
        "employees": [
            {
                ...
                "deductions": [{"type": "health_insurance", "amount": 150.00}],
            }
        ],
        "calculation_rules": {
    ...
    if "calculation_id" in result:
        ...
        if "employee_details" in result:
            ...
            for detail in result["employee_details"]:
                print(
                    f"- {detail.get('employee_name', 'N/A')}: R$ {detail.get('net_salary', 0):,.2f}"
                )
    ...
    if "records_processed" in result:
        ...
        if result.get("errors"):
            ...
            for error in result["errors"][:3]:  # Mostrar apenas os primeiros 3
                print(
                    f"- Linha {error.get('line', 'N/A')}: {error.get('message', 'N/A')}"
                )
