...
class DocumentOCR:
    ...
    def extract_structured_data(
        self, text: str, document_type: str = "payroll"
    ) -> Dict:
        """
    ...
    for file_path in sample_files:
        ...
        for preprocess in [False, True]:
            ...
            result = ocr_processor.extract_text_from_image(
                file_path, preprocess=preprocess
            )
    ...
    for file_path in sample_files:
        ...
        if result["success"]:
            ...
            structured_data = ocr_processor.extract_structured_data(
                result["text"], document_type="payroll"
            )
...
def example_performance_comparison():
    ...
    for config in configs:
        ...
        try:
            ocr_processor = DocumentOCR(lang=config["lang"], use_gpu=config["use_gpu"])
    ...
    if results:
        ...
        for result in results:
            print(
                f"- {result['config']}: {result['time']:.2f}s (conf: {result['confidence']:.2f})"
            )
