...
class TestUserService:
    ...
    def test_create_test_users(self):
        ...
        # Verify gestor A
        gestor_a = next(
            u for u in test_users if u["username"] == DefaultUsernames.GESTOR_A
        )
        ...
        # Verify client X
        client_x = next(
            u for u in test_users if u["username"] == DefaultUsernames.CLIENT_X
        )
    ...
    def test_validate_user_data_missing_field(self):
        ...
        is_valid, error_message = self.user_service.validate_user_data(
            invalid_user_data
        )
...
class TestDatabaseInitializationRefactoring:
    ...
    def test_create_companies_and_users_with_service(self, mock_create_engine):
        ...
        assert (
            len(sql_statements) >= 8
        )  # 2 table creations + 4 companies + 1 admin + 3 test users
        ...
        company_statements = [
            ...
            if f"INSERT INTO {DatabaseTableNames.COMPANIES}" in s
        ]
        ...
        user_statements = [
            s
            for s in sql_statements
            if f"INSERT INTO {DatabaseTableNames.USERS_ENHANCED}" in s
        ]
    ...
    def test_create_notification_templates_with_service(self, mock_create_engine):
        ...
        template_statements = [
            s
            for s in sql_statements
            if f"INSERT INTO {DatabaseTableNames.NOTIFICATION_TEMPLATES}" in s
        ]
...
class TestSecurityImprovements:
    ...
    def test_environment_based_test_credentials(self):
        ...
        with patch.dict(
            os.environ,
            {"TEST_USERNAME": "test_user", "TEST_PASSWORD": "test_pass_123!"},
        ):
            from src.services.auth_service import authenticate_user
...
def test_integration_refactored_components():
    ...
    print(
        "✅ Integration test passed: All refactored components work together correctly"
    )
