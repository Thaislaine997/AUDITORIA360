...
def test_audit_relatorio_performance():
    ...
    try:
        ...
        test_cases = [
            {"params": "?period_start=2024-01-01&period_end=2024-01-31", "name": "Monthly Report"},
        ...
        for case in test_cases:
            ...
            print(f"   📊 {case['name']}: {duration:.3f}s ({'✅' if duration < 1.0 else '⚠️'})")
...
def test_compliance_check_performance():
    ...
    try:
        ...
        test_cases = [
            {"params": "?entity_type=payroll&entity_id=emp001", "name": "Payroll Check"},
            {"params": "?entity_type=employee&entity_id=12345", "name": "Employee Check"},
...
def run_performance_tests():
    ...
    for result in test_results:
        if "error" in result:
            ...
            print(f"{status} {result['endpoint']}: {result['average_time']:.3f}s (target: {result['target']})")
