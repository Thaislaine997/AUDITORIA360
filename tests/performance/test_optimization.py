...
class TestAPIPerformance:
    ...
    async def test_payroll_calculation_async_performance(self):
        ...
        with patch(
            "src.services.payroll_service.get_payroll_competency_by_id"
        ) as mock_get_comp, patch(
            "src.services.payroll_service.get_employees_with_payroll_async"
        ) as mock_get_emp:
...
class TestPerformanceTargets:
    ...
    def test_memory_efficiency(self):
        ...
        assert (
            retrieval_time < 0.2
        ), f"Large object retrieval too slow: {retrieval_time}s"
...
class TestOverallPerformanceImprovement:
    ...
    def test_60_percent_improvement_simulation(self):
        ...
        assert (
            improvement >= 60
        ), f"Performance improvement {improvement:.1f}% below 60% target"
