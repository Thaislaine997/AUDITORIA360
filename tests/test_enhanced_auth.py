...
class TestEnhancedAuthentication:
    ...
    def test_data_segregation_authorization(self):
        ...
        assert self.auth_manager.authorize_data_access(cliente, "empresa", "EMPRESA_X")
        assert not self.auth_manager.authorize_data_access(
...
class TestUIAccessControl:
    ...
    def test_contabilidade_ui_restrictions(self):
        ...
        allowed_modules = [
            "meus_clientes",
            "gestao_folha",
            "relatorios_segmentados",
            "consultor_riscos",
        ]

        # Should be restricted from global modules
        restricted_modules = ["outras_contabilidades", "administracao_global"]
    ...
    def test_cliente_final_ui_restrictions(self):
        ...
        restricted_modules = ["outras_empresas", "administracao", "gestao_usuarios"]
...
class TestIntegrationScenarios:
    ...
    async def test_complete_super_admin_workflow(self):
        ...
        for empresa in ["EMPRESA_X", "EMPRESA_Y", "EMPRESA_Z"]:
            assert self.auth_manager.authorize_data_access(user, "empresa", empresa)
    ...
    async def test_complete_contabilidade_workflow(self):
        ...
        assert self.auth_manager.authorize_data_access(user, "client", "EMPRESA_X")
        assert self.auth_manager.authorize_data_access(user, "client", "EMPRESA_Y")
    ...
    async def test_complete_cliente_final_workflow(self):
        ...
        assert self.auth_manager.authorize_data_access(user, "empresa", "EMPRESA_X")

        # 3. Cannot access other enterprises
        assert not self.auth_manager.authorize_data_access(user, "empresa", "EMPRESA_Y")
        assert not self.auth_manager.authorize_data_access(user, "empresa", "EMPRESA_Z")
