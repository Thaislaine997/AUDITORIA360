...
from src.schemas.parametros_legais_schemas import FaixaIRRF, TabelaIRRF
...
def get_irrf_payload(
    ...
    if faixas is None:
        faixas = [
            ...
            },  # Última faixa sem base_calculo_ate
        ]
    payload = {
...
def test_criar_irrf_campos_obrigatorios_ausentes(
...
def test_criar_irrf_faixa_sem_parcela_deduzir(client):
    ...
    payload["faixas"] = [
        {"base_calculo_de": 0, "aliquota": 0}
    ]  # parcela_a_deduzir faltando
    response = client.post("/param-legais/irrf/", json=payload)
    assert response.status_code == 422
...
def test_criar_irrf_deducao_dependente_negativa(client):
    payload = get_irrf_payload(
        data_inicio_vigencia=date(2025, 4, 1), deducao_por_dependente=-10.0
    )
    response = client.post("/param-legais/irrf/", json=payload)
    assert response.status_code == 422
...
def test_obter_tabela_irrf_por_id(client):
    created_id = client.post(
        "/param-legais/irrf/", json=get_irrf_payload(date(2026, 1, 1))
    ).json()["id_versao"]
...
def test_atualizar_tabela_irrf(client):
    id_v1 = client.post(
        "/param-legais/irrf/",
        json=get_irrf_payload(date(2027, 1, 1), deducao_por_dependente=180.0),
    ).json()["id_versao"]
    payload_update = {
        "data_inicio_vigencia": date(2027, 6, 1).isoformat(),
        "deducao_por_dependente": 190.0,
        "faixas": [
            {"base_calculo_de": 0, "aliquota": 0, "parcela_a_deduzir": 0}
        ],  # Simplificado
        "observacao": "IRRF V2",
    }
    ...
    assert (
        v1_after_update["data_fim_vigencia"]
...
def test_inativar_tabela_irrf(client):
    created_id = client.post(
        "/param-legais/irrf/", json=get_irrf_payload(date(2028, 1, 1))
    ).json()["id_versao"]
...
def test_obter_tabela_irrf_vigente(client):
    client.post(
        "/param-legais/irrf/",
        json=get_irrf_payload(
            date(2023, 1, 1),
            deducao_por_dependente=170,
    ...
    client.post(
        ...
        json=get_irrf_payload(
            date(2024, 1, 1),
            deducao_por_dependente=180,
            data_fim_vigencia=date(2024, 12, 31),
        ),
    )
    client.post(
        "/param-legais/irrf/",
        json=get_irrf_payload(date(2025, 1, 1), deducao_por_dependente=190),
    )
    ...
    assert (
        client.get("/param-legais/irrf/vigente?data_referencia=2022-01-01").status_code
        == 404
    )
