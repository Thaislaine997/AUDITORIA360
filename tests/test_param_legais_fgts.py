...
from services.core.parametros_legais_schemas import TabelaFGTS
...
def test_criar_fgts_campos_obrigatorios_ausentes(client, field_to_remove, expected_detail_part):
...
@pytest.mark.parametrize(
    ...
    [
        ("aliquota_deposito_mensal", -1.0, "must be greater than or equal to 0"), # Ajustar msg
        ("aliquota_multa_rescisoria_saldo", 101.0, "must be less than or equal to 100"), # Ajustar msg
        ("aliquota_contribuicao_social", -5.0, "must be greater than or equal to 0") # Ajustar msg
    ]
)
def test_criar_fgts_aliquotas_invalidas(client, field_to_test, invalid_value, expected_error_msg_part):
    ...
    assert expected_error_msg_part in response.text.lower() # Ajustar conforme a mensagem real
...
def test_obter_parametros_fgts_por_id_nao_encontrado(client):
    ...
    assert "Parâmetros FGTS não encontrados" in response_get.json()["detail"] # Ajustar msg

# Testes de Atualização (PUT)

def test_atualizar_parametros_fgts(client):
    payload_v1 = get_fgts_payload(data_inicio_vigencia=date(2027, 1, 1), aliquota_deposito_mensal=7.0, observacao="FGTS V1")
...
def test_atualizar_parametros_fgts_nao_encontrado(client):
    ...
    response_update = client.put(f"/param-legais/fgts/id_inexistente_fgts_put", json=payload_update)
    assert response_update.status_code == 404
    assert "Parâmetros FGTS não encontrados" in response_update.json()["detail"] # Ajustar msg
...
def test_inativar_parametros_fgts_nao_encontrado(client):
    ...
    assert "Parâmetros FGTS não encontrados" in response_delete.json()["detail"] # Ajustar msg

# Teste de consulta de vigente

def test_obter_parametros_fgts_vigentes_em_data_especifica(client):
    client.post("/param-legais/fgts/", json=get_fgts_payload(date(2023,1,1), aliquota_deposito_mensal=7.0, data_fim_vigencia=date(2023,12,31)))
    client.post("/param-legais/fgts/", json=get_fgts_payload(date(2024,1,1), aliquota_deposito_mensal=8.0, data_fim_vigencia=date(2024,12,31)))
    client.post("/param-legais/fgts/", json=get_fgts_payload(date(2025,1,1), aliquota_deposito_mensal=8.5))
    ...
    response_futuro = client.get("/param-legais/fgts/vigente?data_referencia=2025-07-15")
    assert response_futuro.status_code == 200
    assert response_futuro.json()["aliquota_deposito_mensal"] == 8.5
    
    response_passado_sem_vigencia = client.get("/param-legais/fgts/vigente?data_referencia=2022-01-01")
    assert response_passado_sem_vigencia.status_code == 404
    assert "Nenhum Parâmetro FGTS vigente encontrado" in response_passado_sem_vigencia.json()["detail"] # Ajustar msg