...
class TestAuditRouterFunctions:
    ...
    @pytest.mark.asyncio
    async def test_execute_audit_regular_user_forbidden(self, mock_regular_user, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_list_audit_executions_with_custom_pagination(self, mock_admin_user, mock_db):
    ...
    async def test_list_audit_findings_success(self, mock_admin_user, mock_db):
        ...
        result = await list_audit_findings(
            skip=0, limit=100, severity=None, resolved=None, 
            current_user=mock_admin_user, db=mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_list_audit_findings_with_severity_filter(self, mock_admin_user, mock_db):
        """Test list_audit_findings with severity filter"""
        result = await list_audit_findings(
            skip=0, limit=100, severity="high", resolved=None,
            current_user=mock_admin_user, db=mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_list_audit_findings_with_resolved_filter(self, mock_admin_user, mock_db):
        """Test list_audit_findings with resolved filter"""
        result = await list_audit_findings(
            skip=0, limit=100, severity=None, resolved=True,
            current_user=mock_admin_user, db=mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_list_audit_findings_with_both_filters(self, mock_admin_user, mock_db):
        """Test list_audit_findings with both filters"""
        result = await list_audit_findings(
            skip=5, limit=25, severity="critical", resolved=False,
            current_user=mock_admin_user, db=mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_create_compliance_rule_contador_forbidden(self, mock_contador_user, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_create_compliance_rule_regular_user_forbidden(self, mock_regular_user, mock_db):
    ...
    async def test_list_compliance_reports_contador(self, mock_contador_user, mock_db):
        ...
        result = await list_compliance_reports(current_user=mock_contador_user, db=mock_db)
        
        assert "message" in result
    
    @pytest.mark.asyncio
    async def test_list_compliance_reports_regular_user(self, mock_regular_user, mock_db):
        """Test list_compliance_reports with regular user"""
        result = await list_compliance_reports(current_user=mock_regular_user, db=mock_db)
    ...
    async def test_list_audit_executions_parameter_combinations(self, mock_admin_user, mock_db, skip, limit):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("severity,resolved", [
        ("low", True),
        ("medium", False),
        ("high", None),
        ("critical", True),
    ...
    async def test_list_audit_findings_filter_combinations(self, mock_admin_user, mock_db, severity, resolved):
        """Test list_audit_findings with various filter combinations"""
        result = await list_audit_findings(
            skip=0, limit=100, severity=severity, resolved=resolved,
            current_user=mock_admin_user, db=mock_db
        )
    ...
    async def test_execute_audit_role_access_matrix(self, mock_db, role, should_succeed):
    ...
    async def test_list_functions_all_roles_allowed(self, mock_db, role):
        ...
        result1 = await list_audit_executions(skip=0, limit=100, current_user=user, db=mock_db)
        result2 = await list_audit_findings(skip=0, limit=100, severity=None, resolved=None, current_user=user, db=mock_db)
    ...
    async def test_create_compliance_rule_role_access_matrix(self, mock_db, role, should_succeed):
