...
class TestOCRIntegration:
    ...
    def mock_complex_ocr_result(self):
        ...
        return [
            [
                ...
                ([[10, 12], [13, 14], [15, 16], [17, 18]], ('Relatório de Folha', 0.92)),
                ([[20, 22], [23, 24], [25, 26], [27, 28]], ('Janeiro 2025', 0.85)),
                ([[30, 32], [33, 34], [35, 36], [37, 38]], ('Total: R$ 150.000,00', 0.96))
            ]
        ]

    # Test basic OCR functionality
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_success(self, mock_paddleocr_class, sample_image_path, mock_ocr_result):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_empty_result(self, mock_paddleocr_class, sample_image_path, mock_empty_ocr_result):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_complex_result(self, mock_paddleocr_class, sample_image_path, mock_complex_ocr_result):
        ...
        # Assertions
        expected_text = "AUDITORIA360\nRelatório de Folha\nJaneiro 2025\nTotal: R$ 150.000,00"
        assert result == expected_text
    
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_single_line(self, mock_paddleocr_class, sample_image_path):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_invalid_image(self, mock_paddleocr_class, sample_image_path):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_paddleocr_initialization_error(self, mock_paddleocr_class, sample_image_path):
        """Test OCR when PaddleOCR initialization fails"""
        mock_paddleocr_class.side_effect = Exception("Erro na inicialização do PaddleOCR")
        
        with pytest.raises(Exception):
            extrair_texto_ocr(sample_image_path)
    
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_malformed_result(self, mock_paddleocr_class, sample_image_path):
    ...
    @pytest.mark.parametrize("file_extension", ['.png', '.jpg', '.jpeg', '.tiff', '.bmp'])
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_configuration(self, mock_paddleocr_class, sample_image_path, mock_ocr_result):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_special_characters(self, mock_paddleocr_class, sample_image_path):
        """Test OCR with special characters and accents"""
        special_chars_result = [
            [
                ([[1, 2], [3, 4], [5, 6], [7, 8]], ('João José da Silva', 0.95)),
                ([[10, 12], [13, 14], [15, 16], [17, 18]], ('Salário: R$ 3.500,50', 0.92)),
                ([[20, 22], [23, 24], [25, 26], [27, 28]], ('Função: Técnico em Informática', 0.88)),
                ([[30, 32], [33, 34], [35, 36], [37, 38]], ('E-mail: joao@empresa.com.br', 0.93))
            ]
        ]
        ...
        expected_text = ("João José da Silva\n"
                        "Salário: R$ 3.500,50\n"
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_memory_efficiency(self, mock_paddleocr_class, sample_image_path):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_very_long_path(self, mock_paddleocr_class, mock_ocr_result):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_whitespace_handling(self, mock_paddleocr_class, sample_image_path):
    ...
    @patch('services.ocr_utils.PaddleOCR')
    def test_extrair_texto_ocr_empty_text_lines(self, mock_paddleocr_class, sample_image_path):
...
class TestOCRPipelineIntegration:
    ...
    def test_ocr_pipeline_document_processing(self):
        ...
        with patch('services.ocr_utils.PaddleOCR') as mock_paddleocr_class:
            ...
            payroll_result = [
                [
                    ([[0, 0], [100, 0], [100, 20], [0, 20]], ('FOLHA DE PAGAMENTO', 0.98)),
                    ([[0, 25], [80, 25], [80, 40], [0, 40]], ('Janeiro/2025', 0.95)),
                    ([[0, 50], [60, 50], [60, 65], [0, 65]], ('Nome: João Silva', 0.93)),
                    ([[0, 70], [70, 70], [70, 85], [0, 85]], ('Cargo: Analista', 0.91)),
                    ([[0, 90], [90, 90], [90, 105], [0, 105]], ('Salário: R$ 5.000,00', 0.96)),
                    ([[0, 110], [85, 110], [85, 125], [0, 125]], ('INSS: R$ 550,00', 0.94)),
                    ([[0, 130], [80, 130], [80, 145], [0, 145]], ('IRRF: R$ 427,63', 0.92)),
                ]
            ]
