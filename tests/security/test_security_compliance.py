...
class TestSecretsManager:
    ...
    def test_secret_key_validation(self):
        ...
        with patch.dict(
            os.environ,
            {"SECRET_KEY": "a" * 31, "ENVIRONMENT": "production"},
            clear=True,
        ):
            manager = SecretsManager()
            with pytest.raises(
                ValueError,
                match="SECRET_KEY must be at least 32 characters in production",
            ):
...
class TestInputValidation:
    ...
    def test_user_input_sanitization(self):
        ...
        assert "nested" not in sanitized or "DROP TABLE" not in str(
...
class TestSecurityCompliance:
    ...
    def test_no_database_credentials_in_code(self):
        ...
        assert all(
            "return os.getenv" in line
            or "SimpleSecretsManager"
            in content[content.find(line) - 200 : content.find(line)]
            for line in lines_with_postgres
