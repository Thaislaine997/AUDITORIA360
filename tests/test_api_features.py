    ...
    try:
        ...
        if response.status_code == 200:
            ...
            response = requests.get(
                f"{BASE_URL}/api/v1/reports/block-types/available", timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Available block types: {len(block_types.get('block_types', []))}"
                )
...
def test_notifications():
    ...
    try:
        # Test unread count
        response = requests.get(
            f"{BASE_URL}/api/v1/notifications/unread-count", timeout=5
        )
...
def main():
    ...
    # Run feature tests
    tests = [test_report_templates, test_notifications, test_documents_export]
