...
def streamlit_server(
    auto_mock_painel_dependencies,
):  # Depende dos mocks para que o painel inicie corretamente
    """
    ...
    try:
        ...
        def dynamic_login_implementation(form_name, location="main"):
            ...
            if e2e_context_instance.username and _mock_st:
                ...
                user_config = mock_yaml_config_content["credentials"]["usernames"].get(
                    username
                )
                ...
                _mock_st
            ):  # Fallback para testuser se e2e_context_instance não estiver preenchido
        ...
        p_stauth = patch.dict(
            sys.modules, {"streamlit_authenticator": mock_stauth_module}
        )
        ...
        mock_st.session_state.get = MagicMock(
            side_effect=lambda key, default=None: _session_state_dict.get(key, default)
        )

        def mock_text_input_implementation(label, value="", **kwargs):
            current_label = str(label).lower()

            if "username" in current_label or (
                kwargs.get("key") and "username" in str(kwargs.get("key")).lower()
            ):
                return (
                    e2e_context_instance.username
                    if e2e_context_instance.username is not None
                    else "testuser_from_text_input"
                )
            elif "password" in current_label or (
                kwargs.get("key") and "password" in str(kwargs.get("key")).lower()
            ):
                return (
                    e2e_context_instance.password
                    if e2e_context_instance.password is not None
                    else "password_from_text_input"
                )
            return value
        ...
        p_gcloud = patch.dict(
            sys.modules,
            {
                ...
                "google.cloud": MagicMock(
                    bigquery=mock_bigquery_module, storage=mock_storage_module
                ),
