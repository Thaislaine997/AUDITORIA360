...
from src.schemas.parametros_legais_schemas import TabelaFGTS, TabelaSalarioFamilia
...
def test_criar_sf_campos_obrigatorios_ausentes(
...
def test_obter_tabela_sf_por_id(client):
    created_id = client.post(
        "/param-legais/salario-familia/", json=get_sf_payload(date(2026, 1, 1))
    ).json()["id_versao"]
...
def test_atualizar_tabela_sf(client):
    id_v1 = client.post(
        "/param-legais/salario-familia/", json=get_sf_payload(date(2027, 1, 1))
    ).json()["id_versao"]
    payload_update = {
    ...
    response_update = client.put(
        f"/param-legais/salario-familia/{id_v1}", json=payload_update
    )
...
def test_inativar_tabela_sf(client):
    created_id = client.post(
        "/param-legais/salario-familia/", json=get_sf_payload(date(2028, 1, 1))
    ).json()["id_versao"]
    assert (
        client.delete(f"/param-legais/salario-familia/{created_id}").status_code == 200
    )
    assert (
        client.get(f"/param-legais/salario-familia/{created_id}").json()["ativo"]
        is False
    )
...
def test_obter_tabela_sf_vigente(client):
    client.post(
        "/param-legais/salario-familia/",
        json=get_sf_payload(
            date(2023, 1, 1),
            faixas=[{"valor_renda_maxima": 1700, "valor_quota": 50}],
    ...
    client.post(
        ...
        json=get_sf_payload(
            date(2024, 1, 1),
            faixas=[{"valor_renda_maxima": 1800, "valor_quota": 60}],
    ...
    client.post(
        "/param-legais/salario-familia/",
        json=get_sf_payload(
            date(2025, 1, 1), faixas=[{"valor_renda_maxima": 1900, "valor_quota": 70}]
        ),
    )

    response = client.get(
        "/param-legais/salario-familia/vigente?data_referencia=2024-06-30"
    )
    assert response.status_code == 200
    ...
    assert (
        client.get(
            "/param-legais/salario-familia/vigente?data_referencia=2022-01-01"
        ).status_code
        == 404
    )
