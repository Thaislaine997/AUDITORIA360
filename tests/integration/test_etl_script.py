...
try:
    ...
    from etl_elt import main as etl_main
    from etl_elt import (
...
class TestETLScriptIntegration:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("etl_elt.bigquery.Client") as mock_client:
                ...
                mock_client.assert_called_once_with(
                    project=None
                )  # PROJECT_ID would be None without env
    ...
    def test_get_bigquery_client_with_credentials(self):
        ...
        with patch.dict(os.environ, {"GOOGLE_APPLICATION_CREDENTIALS": test_key_path}):
            with patch(
                "etl_elt.service_account.Credentials.from_service_account_file"
            ) as mock_creds:
                with patch("etl_elt.bigquery.Client") as mock_client:
    ...
    def test_transformar_features(self, mock_dataframe):
        ...
        if hasattr(result, "data"):  # MockDataFrame
            ...
            assert all(
                abs(a - b) < 0.001
                for a, b in zip(result["proporcao_descontos"].tolist(), expected_prop)
            )
    ...
    def test_etl_environment_variables(self):
        ...
        with patch.dict(
            os.environ, {"GCP_PROJECT_ID": test_project, "BQ_DATASET_ID": test_dataset}
        ):
    ...
    def test_etl_configuration_flexibility(self, project_id, dataset_id):
        ...
        with patch.dict(
            os.environ, {"GCP_PROJECT_ID": project_id, "BQ_DATASET_ID": dataset_id}
        ):
