...
def test_validation_functions():
    ...
    try:
        from src.api.common.validators import (
            validate_cnpj,
            validate_cpf,
            validate_email,
        )
...
def test_response_models():
    ...
    try:
        ...
        paginated_resp = create_paginated_response(
            items=[{"item": 1}, {"item": 2}], page=1, page_size=2, total_items=10
        )
        ...
        error_resp = create_error_response(
            error_code=ErrorCode.INVALID_INPUT, message="Validation failed"
        )
