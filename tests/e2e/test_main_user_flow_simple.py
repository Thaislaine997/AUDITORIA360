...
class TestMainUserFlowSimulated:
    ...
    def test_pontos_module_removal_validation(self):
        ...
        filtered_files = [f for f in pontos_files 
                         if not any(legit in f for legit in legitimate_files)]
        
        assert len(filtered_files) == 0, f"Found Pontos-related files that should be removed: {filtered_files}"
