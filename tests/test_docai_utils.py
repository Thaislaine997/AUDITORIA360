...
def test_get_docai_client_with_key_path(
    mock_creds_from_file, mock_docai_constructor, mock_get_logger_client
):
...
def test_list_gcs_pdfs_success(mock_get_logger_list_gcs):
    ...
    mock_gcs_client_constructor_function = MagicMock(
        return_value=mock_storage_client_instance
    )
    print("    Mock do construtor do cliente GCS criado")
    blob1_pdf = MagicMock(spec=RealStorageBlob)
    blob1_pdf.name = "folder/doc1.pdf"
    blob1_pdf.content_type = "application/pdf"
    blob2_txt = MagicMock(spec=RealStorageBlob)
    blob2_txt.name = "folder/doc2.txt"
    blob2_txt.content_type = "text/plain"
...
def test_list_gcs_pdfs_correct_filtering(mock_get_logger_filter):
    ...
    blob1 = MagicMock(spec=RealStorageBlob)
    blob1.name = "folder/doc1.pdf"
    blob1.content_type = "application/pdf"
    blob2 = MagicMock(spec=RealStorageBlob)
    blob2.name = "folder/doc2.txt"
    blob2.content_type = "text/plain"
    blob3 = MagicMock(spec=RealStorageBlob)
    blob3.name = "folder/DOC4.PDF"
    blob3.content_type = "application/pdf"
    blob4 = MagicMock(spec=RealStorageBlob)
    blob4.name = "folder/image.jpg"
    blob4.content_type = "image/jpeg"
    mock_storage_client_instance.list_blobs.return_value = [blob1, blob2, blob3, blob4]
    bucket_name = "test-bucket-filter"
    prefix_folder = "folder/"
    env_vars = {
        "GCP_PROJECT_ID": "p",
        "GCP_LOCATION": "l",
...
def test_list_gcs_pdfs_no_pdfs_found(mock_get_logger_no_pdf):
    ...
    blob_txt = MagicMock(spec=RealStorageBlob)
    blob_txt.name = "file.txt"
...
def test_list_gcs_pdfs_client_exception(mock_get_logger_exc):
    ...
    mock_gcs_client_constructor_function = MagicMock(
        side_effect=Exception("Falha ao criar cliente GCS")
    )
