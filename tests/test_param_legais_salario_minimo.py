...
def get_sm_payload(
    ...
    if valores_regionais:
        payload["valores_regionais"] = (
            valores_regionais  # API espera um dict que será convertido para JSON
        )
    return payload
...
def test_criar_novo_salario_minimo(client):
    payload = get_sm_payload(
        data_inicio_vigencia=date(2025, 1, 1), valor_nacional=1500.00
    )
...
def test_criar_salario_minimo_campos_obrigatorios_ausentes(
    client, field_to_remove, expected_detail_part
):
    payload = get_sm_payload(
        data_inicio_vigencia=date(2025, 3, 1), valor_nacional=1520.00
    )
...
def test_criar_salario_minimo_valor_nacional_negativo(client):
    payload = get_sm_payload(
        data_inicio_vigencia=date(2025, 4, 1), valor_nacional=-100.00
    )
    response = client.post("/param-legais/salario-minimo/", json=payload)
    assert response.status_code == 422
    ...
    assert (
        "must be greater than 0" in response.text.lower()
...
def test_criar_salario_minimo_data_fim_anterior_inicio(client):
    ...
    )  # Ou 400 dependendo da validação (schema vs. lógica de negócios)
...
def test_listar_salarios_minimos(client):
    ...
    client.post(
        "/param-legais/salario-minimo/", json=get_sm_payload(date(2024, 1, 1), 1400)
    )
    client.post(
        "/param-legais/salario-minimo/", json=get_sm_payload(date(2024, 5, 1), 1450)
    )
...
def test_obter_salario_minimo_por_id(client):
    payload_create = get_sm_payload(
        data_inicio_vigencia=date(2026, 1, 1), valor_nacional=1600.00
    )
...
def test_atualizar_salario_minimo(client):
    ...
    payload_v1 = get_sm_payload(
        data_inicio_vigencia=date(2027, 1, 1),
        valor_nacional=1700.00,
        observacao="Versão 1",
    )
    ...
    response_update = client.put(
        f"/param-legais/salario-minimo/{id_v1}", json=payload_update
    )
...
def test_atualizar_salario_minimo_nao_encontrado(client):
    ...
    response_update = client.put(
        f"/param-legais/salario-minimo/id_inexistente_put", json=payload_update
    )
...
def test_inativar_salario_minimo(client):
    payload_create = get_sm_payload(
        data_inicio_vigencia=date(2028, 1, 1), valor_nacional=1800.00
    )
    response_create = client.post("/param-legais/salario-minimo/", json=payload_create)
    assert response_create.status_code == 201
    created_id = response_create.json()["id_versao"]
    ...
    response_get_before_delete = client.get(
        f"/param-legais/salario-minimo/{created_id}"
    )
...
def test_inativar_salario_minimo_nao_encontrado(client):
    response_delete = client.delete(
        f"/param-legais/salario-minimo/id_inexistente_delete"
    )
...
def test_obter_salario_minimo_vigente_em_data_especifica(client):
    ...
    client.post(
        "/param-legais/salario-minimo/",
        json=get_sm_payload(
            date(2023, 1, 1), 1300, data_fim_vigencia=date(2023, 12, 31)
        ),
    )
    client.post(
        "/param-legais/salario-minimo/",
        json=get_sm_payload(
            date(2024, 1, 1), 1400, data_fim_vigencia=date(2024, 12, 31)
        ),
    )
    client.post(
        "/param-legais/salario-minimo/", json=get_sm_payload(date(2025, 1, 1), 1500)
    )  # Vigente atualmente

    response = client.get(
        "/param-legais/salario-minimo/vigente?data_referencia=2024-07-15"
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response_futuro = client.get(
        "/param-legais/salario-minimo/vigente?data_referencia=2025-07-15"
    )
    ...
    response_passado_sem_vigencia = client.get(
        "/param-legais/salario-minimo/vigente?data_referencia=2022-01-01"
    )
    assert (
        response_passado_sem_vigencia.status_code == 404
    )  # Ou retorna None/lista vazia dependendo da API
    assert (
        "Nenhum Salário Mínimo vigente encontrado"
...
async def test_criar_salario_minimo_conflito_vigencia_total(client: AsyncClient):
    ...
    payload_v1 = get_sm_payload(
        data_inicio_vigencia=date(2029, 1, 1),
        valor_nacional=2000.00,
    ...
    payload_conflito = get_sm_payload(
        data_inicio_vigencia=date(2029, 1, 1),
        valor_nacional=2100.00,
        data_fim_vigencia=date(2029, 12, 31),
    )
    response_conflito = await client.post(
        "/param-legais/salario-minimo/", json=payload_conflito
    )
...
@pytest.mark.asyncio
async def test_criar_salario_minimo_conflito_vigencia_parcial_inicio(
    client: AsyncClient,
):
    # Criar uma tabela que cobre um período
    payload_v1 = get_sm_payload(
        data_inicio_vigencia=date(2030, 6, 1),
        valor_nacional=2200.00,
        data_fim_vigencia=date(2030, 12, 31),
    )
    response_v1 = await client.post("/param-legais/salario-minimo/", json=payload_v1)
    assert response_v1.status_code == status.HTTP_201_CREATED
