...
from src.schemas.parametros_legais_schemas import FaixaINSS, TabelaINSS
...
def get_inss_payload(
    ...
    if faixas is None:
        faixas = [
            ...
            {
                "valor_inicial": 4000.04,
                "valor_final": valor_teto_contribuicao,
                "aliquota": 14.0,
            },  # Última faixa até o teto
        ]
    payload = {
...
def test_criar_inss_campos_obrigatorios_ausentes(
...
def test_criar_inss_valor_teto_negativo(client):
    payload = get_inss_payload(
        data_inicio_vigencia=date(2025, 4, 1), valor_teto_contribuicao=-100.00
    )
    response = client.post("/param-legais/inss/", json=payload)
    assert response.status_code == 422
...
def test_criar_inss_data_fim_anterior_inicio(client):
    payload = get_inss_payload(
        data_inicio_vigencia=date(2025, 5, 1), data_fim_vigencia=date(2025, 4, 30)
    )
    response = client.post("/param-legais/inss/", json=payload)
    assert response.status_code == 422
...
def test_atualizar_tabela_inss(client):
    payload_v1 = get_inss_payload(
        data_inicio_vigencia=date(2027, 1, 1), valor_teto_contribuicao=7500.00
    )
    ...
    payload_update = {
        ...
        "faixas": [
            {"valor_inicial": 0, "valor_final": 1500, "aliquota": 7.0}
        ],  # Faixas simplificadas para teste
...
def test_atualizar_tabela_inss_nao_encontrada(client):
    ...
    response_update = client.put(
        f"/param-legais/inss/id_inss_inexistente_put", json=payload_update
    )
...
def test_obter_tabela_inss_vigente_em_data_especifica(client):
    client.post(
        "/param-legais/inss/",
        json=get_inss_payload(
            date(2023, 1, 1),
            valor_teto_contribuicao=7000,
    ...
    client.post(
        ...
        json=get_inss_payload(
            date(2024, 1, 1),
            valor_teto_contribuicao=7500,
            data_fim_vigencia=date(2024, 12, 31),
        ),
    )
    client.post(
        "/param-legais/inss/",
        json=get_inss_payload(date(2025, 1, 1), valor_teto_contribuicao=8000),
    )
    ...
    response_passado_sem_vigencia = client.get(
        "/param-legais/inss/vigente?data_referencia=2022-01-01"
    )
    assert response_passado_sem_vigencia.status_code == 404
    assert (
        "Nenhuma Tabela INSS vigente encontrada"
