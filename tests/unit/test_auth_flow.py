...
class TestAuthFlow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(
            os.path.exists(self.login_yaml_path),
    ...
    def test_login_yaml_structure(self):
        ...
        self.assertIn(
            "credentials", config, "login.yaml deve ter uma chave 'credentials'"
        )
        self.assertIn(
            "usernames",
            config["credentials"],
            "credentials deve ter uma chave 'usernames'",
        )
        self.assertIn("cookie", config, "login.yaml deve ter uma chave 'cookie'")

        # Verificar se há pelo menos um usuário
        self.assertTrue(
            len(config["credentials"]["usernames"]) > 0,
        ...
        for username, user_data in config["credentials"]["usernames"].items():
            self.assertIn(
                "password", user_data, f"Usuário {username} deve ter uma senha"
            )
            self.assertIn(
                "client_id", user_data, f"Usuário {username} deve ter um client_id"
            )
    ...
    def test_api_authentication(self, mock_post):
        ...
        self.assertIn(
            "/auth/token", args[0], "O endpoint de autenticação deve ser '/auth/token'"
        )
        ...
        with patch("streamlit.error") as mock_st_error:
            ...
            self.assertIsNone(
                token, "autenticar_api deve retornar None em caso de erro"
            )
