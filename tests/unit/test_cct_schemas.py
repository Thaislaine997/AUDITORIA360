import pytest
from src.schemas.cct_schemas import ClausulaExtraidaResponse, UpdateClausulaRevisaoRequest, CCTIdentificacaoResponse, CCTDetalheResponse, CCTStatusResponse, CCTUpdateStatusRequest
...
def test_update_clausula_revisao_request():
    obj = UpdateClausulaRevisaoRequest(revisado=True, texto_revisado="Novo texto", comentarios="OK")
    assert obj.revisado is True

def test_cct_identificacao_response():
    obj = CCTIdentificacaoResponse(id_cct="cct1", vigencia_inicio=date(2025,1,1), vigencia_fim=date(2025,12,31))
    assert obj.id_cct == "cct1"

def test_cct_detalhe_response():
    obj = CCTDetalheResponse(id_cct="cct1", texto_integral="Texto", status_processamento="Processado")
