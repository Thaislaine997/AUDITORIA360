...
def test_prever_rubrica_com_vertex_sucesso(mocker):
    ...
    mock_aiplatform_endpoint_class = mocker.patch(
        "src.vertex_utils.aiplatform.Endpoint", autospec=True
    )
    ...
    mock_aiplatform_endpoint_class.assert_called_once_with(
        endpoint_name=expected_endpoint_name
    )
    mock_endpoint_instance.predict.assert_called_once_with(
        instances=[{"content": texto_clausula_exemplo}]
    )
...
def test_prever_rubrica_com_vertex_resposta_malformada(mocker):
    ...
    mocker.patch("src.vertex_utils.inicializar_vertex")
    mock_endpoint_instance = MagicMock()
    mock_predict_response = MagicMock()
