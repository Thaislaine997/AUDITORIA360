...
def test_classificar_clausula_resposta_json_simples(mocker): # mocker é fornecido pelo pytest-mock
...
def test_classificar_clausula_resposta_json_decode_error(mocker):
    ...
    resposta_modelo_simulada_texto = "Isso parece ser uma cláusula genérica, não um JSON." # Não é JSON
    mock_resposta_gemini = MockGeminiResponse(resposta_modelo_simulada_texto)
...
def test_classificar_clausula_resposta_sem_atributo_text(mocker):
    ...
    mock_resposta_sem_text = MagicMock()
    mock_model_instance = MagicMock()
    ...
    assert "object has no attribute 'group'" in resultado["descricao"] or "expected string or bytes-like object" in resultado["descricao"]
...
def test_classificar_clausula_resposta_modelo_retorna_none(mocker):
    ...
    assert resultado["rubrica"] == "Erro"
    assert "Erro ao contatar o Gemini: 'NoneType' object has no attribute 'text'" in resultado["descricao"]
    mock_generative_model_class.assert_called_once_with("gemini-pro")
    mock_model_instance.generate_content.assert_called_once()