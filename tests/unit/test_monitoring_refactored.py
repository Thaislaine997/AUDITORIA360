...
from src.monitoring.alerts import AlertManager, AlertSeverity, Alert
from src.monitoring.health import HealthChecker, HealthCheck
...
class TestHealthChecker:
    ...
    def test_get_overall_health_one_unhealthy(self):
        ...
        checker.last_results = {
            "check1": HealthCheck("check1", "healthy", datetime.now(), 10.0),
            "check2": HealthCheck("check2", "unhealthy", datetime.now(), 50.0, error="Failed")
        }
...
class TestSystemMonitor:
    ...
    def test_get_system_info(self, mock_psutil):
        ...
        mock_psutil.virtual_memory.return_value = Mock(total=16000000000, available=8000000000)
        mock_psutil.disk_usage.return_value = Mock(total=500000000000, free=200000000000)
