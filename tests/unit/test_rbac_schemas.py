import pytest
from src.schemas.rbac_schemas import Token, TokenData, UserBase, UserCreate, UserInDB, UserPublic, Client
...
def test_user_in_db():
    obj = UserInDB(email="a@b.com", nome="Ana", id_usuario="1", hashed_password="hash", ativo=True, papeis=["admin"])
    assert obj.ativo is True

def test_user_public():
    obj = UserPublic(email="a@b.com", nome="Ana", id_usuario="1", ativo=True, papeis=["admin"])
    assert obj.ativo is True
