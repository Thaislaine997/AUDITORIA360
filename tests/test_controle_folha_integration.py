...
async def test_importacao_e_busca_controle_folha(mock_request, monkeypatch):
    ...
    def mock_get_conf(request: Request):
        return {"gcp_project_id": "test-project", "control_bq_dataset_id": "test_dataset"}
    monkeypatch.setattr("src.controllers.controle_folha_controller.config_manager.get_config", mock_get_conf)
    monkeypatch.setattr("src.controllers.controle_folha_controller.bq_loader.get_bigquery_client", lambda: None)
