...
class TestMonitoringEndpoints:
    ...
    def test_business_event_recording(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/v1/monitoring/business-events",
            json=event_data
        )
...
def run_monitoring_validation():
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/api/v1/monitoring/business-events",
            json=event_data
        )
        assert response.status_code == 200
    ...
    print("\nPróximos passos:")
    print("1. Configurar Grafana: docker-compose -f docker-compose.monitoring.yml up -d")
