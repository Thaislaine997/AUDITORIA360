...
def mock_get_current_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "clients": {
            "client_test_1": {"AI_PROVIDER": "vertex", "LOGO_URL": "http://logo.test/1.png"},
            "default_test_client": {"AI_PROVIDER": "gemini", "LOGO_URL": "http://logo.test/default.png"}
        }
    }
...
class MockBigQueryClient:
    ...
    def query(self, query_string: str, job_config=None):
        ...
        if "FROM `test-project-id.test_dataset_id.vw_contabilidades_options`" in query_string:
            return MockQueryJob([
                bigquery.Row(("contabilidade_1", "Contabilidade Teste 1"), {"id": 0, "nome": 1}),
                bigquery.Row(("contabilidade_2", "Contabilidade Teste 2"), {"id": 0, "nome": 1}),
            ])
        return MockQueryJob([])
...
def test_gcs_event_handler_sheet(mock_process_sheet, client: TestClient):
    ...
    json_data = response.json()
    assert json_data["status"] == "success"
    mock_process_sheet.assert_called_once_with(file_name="planilhas-controle/planilha_teste.xlsx", bucket_name="meu-bucket-planilha-teste")

def test_gcs_event_handler_no_payload(client: TestClient):
    response = client.post('/event-handler', headers={"Content-Type": "application/json"})
...
def test_gcs_event_handler_invalid_envelope(client: TestClient):
    ...
    json_data = response.json()
    assert "detail" in json_data
    assert "Formato de envelope de evento inválido." in json_data["detail"] # Corrigido: Mensagem de erro atualizada
