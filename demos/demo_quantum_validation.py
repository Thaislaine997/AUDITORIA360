...
async def run_quick_demo(orchestrator):
    ...
    try:
        ...
        firework = await orchestrator.fireworks.ignite_function('analytics', request_data)
        ...
        print(f"   ✅ Mini tempestade: {storm_results['requests_completed']} requests processados")
    ...
    try:
        ...
        for test_key, test_name in test_names.items():
            if test_key in results['individual_tests']:
                ...
                if test_key == 'massive_decentralized_query' and 'performance_metrics' in test_result:
                    p99 = test_result['performance_metrics']['p99_execution_time_ms']
                    print(f"      • P99: {p99:.1f}ms (meta: <500ms)")
                    
                elif test_key == 'predictive_cold_start' and 'model_performance' in test_result:
