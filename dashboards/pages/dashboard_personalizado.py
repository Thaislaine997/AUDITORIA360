...
from src.frontend.components.dashboard_interativo import injetar_recursos_interativos, criar_html_widget_interativo
from src.frontend.api_client import APIClient
from src.frontend.components.widget_async_loader import (
    load_widget_async, 
    limpar_cache_widget, 
    criar_placeholder_widget
)
from src.frontend.components.streamlit_bidirectional import dashboard_bidirectional_component
    ...
    if st.session_state.modo_dashboard == "visualizar":
        ...
        if preferencias:
            ...
            if "layout_preferencia_id" not in st.session_state:
                ...
                default_layout = next((p for p in preferencias if p["is_default"]), preferencias[0])
                st.session_state.layout_preferencia_id = default_layout["id_preferencia"]
            ...
            layout_selecionado = st.sidebar.selectbox(
                ...
                index=[p["id_preferencia"] for p in preferencias].index(
                    st.session_state.layout_preferencia_id) if st.session_state.layout_preferencia_id in 
                    [p["id_preferencia"] for p in preferencias] else 0
            )
            ...
            st.session_state.layout_preferencia_id = layout_selecionado["id_preferencia"]
            ...
            st.info("Você ainda não tem layouts personalizados. Crie um novo layout na seção 'Configurar Layout'.")
...
def exibir_dashboard(auth_data):
    ...
    if not preferencias:
        st.info("Você ainda não tem um layout personalizado. Crie um na aba 'Configurar Layout'.")
        exibir_dashboard_padrao()
        return
    ...
    if len(preferencias) > 1:
        ...
        with col1:
            opcao_selecionada = st.selectbox(
                ...
                format_func=lambda x: next((p["nome_layout"] for p in preferencias if p["id_preferencia"] == x), x),
                key="layout_selector"
            )
        
        # Encontrar a preferência selecionada
        preferencia_atual = next((p for p in preferencias if p["id_preferencia"] == opcao_selecionada), preferencias[0])
    else:
...
def configurar_dashboard(auth_data):
    ...
    st.caption("Adicione, remova e configure widgets para criar seu dashboard personalizado.")
    ...
    with col1:
        ...
        if st.button("✨ Criar Novo Layout", use_container_width=True):
            ...
            st.success(f"Novo layout '{nome_layout}' iniciado. Adicione widgets abaixo.")
    ...
    with col2:
        ...
        if preferencias:
            ...
            if st.button("✏️ Editar Layout", use_container_width=True):
                # Encontrar a preferência selecionada
                preferencia = next((p for p in preferencias if p["id_preferencia"] == layout_para_editar), None)
                if preferencia:
                    # Carregar no editor
                    st.session_state.editor_config = {
                    ...
                    st.success(f"Layout '{preferencia['nome_layout']}' carregado para edição.")
                    st.rerun()
        else:
    ...
    with col2:
        ...
        with st.expander("➕ Adicionar Widget", expanded=True):
            ...
            selected_widget = st.selectbox(
                ...
                format_func=lambda x: WIDGETS_CATALOG[x].nome if x in WIDGETS_CATALOG else x
            )
            ...
            if selected_widget in WIDGETS_CATALOG:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    coluna = st.number_input("Coluna", min_value=1, max_value=6, value=1)
                with col2:
                    linha = st.number_input("Linha", min_value=1, max_value=10, value=1)
                    
                col1, col2 = st.columns(2)
                with col1:
                    largura = st.number_input("Largura", min_value=1, max_value=6, value=widget_class.tamanho_padrao["largura"])
                with col2:
                    altura = st.number_input("Altura", min_value=1, max_value=4, value=widget_class.tamanho_padrao["altura"])
        ...
        if st.session_state.editor_config["widgets"]:
            with st.expander("⚙️ Configurar Widget"):
                ...
                widget_names = [f"{idx+1}. {WIDGETS_CATALOG[w['id']].nome}" for idx, w in enumerate(widgets)]
                
                selected_idx = st.selectbox(
                    "Selecione um widget para configurar",
                    options=list(range(len(widgets))),
                    format_func=lambda i: widget_names[i] if i < len(widget_names) else "",
                )
                ...
                if selected_idx is not None and selected_idx < len(widgets):
                    ...
                    config_tabs = st.tabs(["Posição e Tamanho", "Configurações Específicas"])
...
def renderizar_editor_layout():
    ...
    for idx, widget in enumerate(st.session_state.editor_config["widgets"]):
        ...
        with st.container():
            st.markdown(f"""<div id="widget_content_widget_{idx}" style="display:none;">""", unsafe_allow_html=True)
...
def renderizar_layout(preferencia):
    ...
    try:
        ...
        if not preferencia.get("configuracao") or not preferencia["configuracao"].get("widgets"):
        ...
        for idx, widget_config in enumerate(preferencia["configuracao"]["widgets"]):
            # Criar um container para o widget
            with st.container():
                st.markdown(f"""<div id="widget_content_{idx}" style="display:none;">""", unsafe_allow_html=True)
...
def exibir_dashboard_padrao():
    ...
    st.caption("Aqui está um layout inicial. Você pode personalizar seu próprio layout na aba 'Configurar Layout'.")
    ...
    with col1:
        ...
        st.markdown(f"<div style='font-size: 2.5rem; color: #1f77b4; text-align: center; font-weight: bold;'>R$ {valor}</div>", unsafe_allow_html=True)
    
    with col2:
        # KPI Média Salarial
        st.markdown("### Média Salarial")
        valor = "{:,.2f}".format(random.uniform(2000, 6000))
        st.markdown(f"<div style='font-size: 2.5rem; color: #2ca02c; text-align: center; font-weight: bold;'>R$ {valor}</div>", unsafe_allow_html=True)
    ...
    for alerta in alertas:
        ...
        st.markdown(f"{cor} **{alerta['descricao']}** - {alerta['tipo']} - {alerta['data']}")
...
def salvar_layout_existente(auth_data, layout_id, layout_data):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Em caso de erro, mostrar mensagem
            st.error(f"Erro ao atualizar layout: {response.status_code} - {response.text}")
            return False
...
def renderizar_dashboard(preferencia):
    ...
    try:
        ...
        if eventos and "action" in eventos:
            if eventos["action"] == "widget_moved" or eventos["action"] == "widget_resized":
                ...
                for widget in preferencia["configuracao"]["widgets"]:
                    if widget["id"] == eventos["widgetId"]:
                        ...
                        st.success(f"Layout atualizado: {widget['id']} movido/redimensionado")
        ...
        st.markdown('<div class="dashboard-grid" id="dashboard-grid">', unsafe_allow_html=True)
        ...
        for idx, widget_config in enumerate(preferencia["configuracao"]["widgets"]):
            ...
            carregamento_assincrono = widget_config.get("carregamento_assincrono", False)
            ...
            with widget_container:
                if carregamento_assincrono:
                    ...
                    def on_widget_loaded(widget_id, config, data):
                        ...
                        if st.session_state.get(f"widget_loading_{widget_id}_{idx}", True):
                            st.session_state[f"widget_loading_{widget_id}_{idx}"] = False
                    
                    # Verificar se já temos dados em cache para este widget
                    if f"widget_data_{widget_id}_{idx}" in st.session_state and not st.session_state.get(f"widget_loading_{widget_id}_{idx}", True):
                        # Usar dados em cache
                        dados = st.session_state[f"widget_data_{widget_id}_{idx}"]
                        widget_class.renderizar(widget_config.get("configs_adicionais"), dados=dados)
                    else:
                        ...
                        load_widget_async(
                            ...
                            load_func=lambda cfg: widget_class.carregar_dados(cfg) if hasattr(widget_class, 'carregar_dados') else {},
                            callback=on_widget_loaded,
                            priority=prioridade,
                            expiry_minutes=preferencia["configuracao"].get("tempo_cache_minutos", 15)
                        )
                else:
    ...
    if layout_selecionado:
        ...
        with col2:
            if st.button("Copiar para Meus Layouts"):
                novo_nome = st.text_input("Nome para o novo layout:", f"Cópia de {layout_selecionado['nome_layout']}")
                
                if st.button("Confirmar Cópia", type="primary"):
                    id_nova_preferencia = gerenciador.copiar_layout_compartilhado(
                        layout_selecionado["id_preferencia"],
                        novo_nome=novo_nome
                    )
