...
def buscar_folhas_processadas_cliente(id_cliente: str, token: Optional[str]) -> list:
    ...
    logger.info(
        f"Buscando folhas processadas: {api_url} com token: {'Sim' if token else 'Não'}"
    )
    ...
    try:
            ...
            try:
                if isinstance(folha_data.get("periodo_referencia"), str):
                    ...
                    if isinstance(dt_obj, date):
                        folha_data["periodo_referencia_display"] = dt_obj.strftime(
                            "%B/%Y"
                        )
                        folha_data["periodo_referencia_date"] = dt_obj
                    else:
                ...
                folha_data["selectbox_label"] = (
                    f'{folha_data["periodo_referencia_display"]} (ID: {folha_data["id_folha_processada"][:8]}...)'
                )
                folhas_formatadas.append(folha_data)
            except ValueError as ve_date:
                logger.error(
                    f"Erro ao parsear data para folha {folha_data.get('id_folha_processada')}: {ve_date}"
                )
                folha_data["periodo_referencia_display"] = "Data Inconsistente"
                folha_data["periodo_referencia_date"] = date(1900, 1, 1)
                folha_data["selectbox_label"] = (
                    f'Data Inconsistente (ID: {folha_data.get("id_folha_processada","ERRO")[:8]}...)'
                )
                folhas_formatadas.append(folha_data)
        ...
        logger.error(
            f"Erro HTTP ao buscar folhas processadas: {e_http.response.status_code} - {e_http.response.text}",
            exc_info=True,
        )
        ...
        st.error(
            f"Erro ao buscar folhas processadas ({e_http.response.status_code}): {detail}"
        )
        return []
...
def mostrar_dashboard_saude_folha():
    ...
    id_cliente_atual = (
        st.session_state.user_info.get("cliente_id")
    ...
    if not id_cliente_atual and "admin" not in user_roles:
        st.warning(
            "Você precisa estar associado a um cliente para acessar esta página."
        )
        st.stop()
    ...
    with st.sidebar:
        ...
        if "admin" in user_roles:
            ...
            with st.expander("Simular Acesso de Cliente"):
                cliente_simulado = st.text_input(
                    "ID do Cliente a simular:", key="admin_simular_cliente"
                )
            ...
            if (
                "cliente_simulado" in st.session_state
                and st.session_state["cliente_simulado"]
            ):
    ...
    if (
        "admin" in user_roles
        and "cliente_simulado" in st.session_state
        and st.session_state["cliente_simulado"]
    ):
        id_cliente_para_api = st.session_state["cliente_simulado"]
        st.warning(
            f"🔄 Visualizando dados como cliente ID: {id_cliente_para_api} (Modo Simulação)"
        )
    else:
    ...
    if id_cliente_para_api:
        folhas_disponiveis = buscar_folhas_processadas_cliente(
            id_cliente_para_api, api_token
        )
    if not folhas_disponiveis:
        st.info(
            "Nenhuma folha processada encontrada para este cliente ou ocorreu um erro ao buscá-las."
        )
        return

    map_label_to_id = {
        f["selectbox_label"]: f["id_folha_processada"] for f in folhas_disponiveis
    }
    label_selecionada = st.selectbox(
        "Selecione o Período da Folha:", options=list(map_label_to_id.keys())
    )
    ...
    folha_selecionada_obj = next(
        (
            f
            for f in folhas_disponiveis
            if f["id_folha_processada"] == id_folha_selecionada
        ),
        None,
    )
    periodo_selecionado_display = (
        folha_selecionada_obj["periodo_referencia_display"]
        if folha_selecionada_obj
    ...
    if st.button("Carregar Dashboard"):
        ...
        try:
            ...
            ):  # Adicionado tratamento com handle_api_error
                handle_api_error(response.status_code)
                st.rerun()
            ...
            logger.error(
                f"Erro HTTP ao buscar dados do dashboard: {e_http.response.status_code} - {e_http.response.text}",
                exc_info=True,
            )
            detail = "Erro desconhecido"
            try:
            ...
            st.error(
                f"Erro ao buscar dados do dashboard ({e_http.response.status_code}): {detail}"
            )
            st.session_state.dashboard_data = None
        except requests.exceptions.RequestException as e_req:
            logger.error(
                f"Erro de requisição ao buscar dados do dashboard: {e_req}",
                exc_info=True,
            )
            ...
            logger.error(
                f"Erro inesperado ao carregar dashboard: {e_gen}", exc_info=True
            )
    ...
    if "dashboard_data" in st.session_state and st.session_state.dashboard_data:
        ...
        st.subheader(
            f"Resultados para: {periodo_selecionado_display} (Folha ID: {id_folha_selecionada})"
        )
        col1, col2, col3, col4, col5 = st.columns(5)
        col1.metric(
            "Total Bruto (Extrato)",
            f"R$ {Decimal(kpis.get('total_bruto_folha_extrato', '0.00')):,.2f}".replace(
                ",", "X"
            )
            .replace(".", ",")
            .replace("X", "."),
        ...
        col2.metric(
            "Total Líquido (Extrato)",
            f"R$ {Decimal(kpis.get('total_liquido_folha_extrato', '0.00')):,.2f}".replace(
                ",", "X"
            )
            .replace(".", ",")
            .replace("X", "."),
        ...
        col3.metric(
            "Nº Funcionários", str(kpis.get("numero_funcionarios_identificados", 0))
        )
        ...
        col5.metric(
            ...
            str(total_div),
            delta_color="inverse" if total_div > 0 else "off",
        )
        st.markdown("---")
        ...
        if div_tipo:
            ...
            df_impacto["soma_diferenca_absoluta"] = df_impacto[
                "soma_diferenca_absoluta"
            ].astype(float)
            if not df_impacto.empty:
                st.bar_chart(
                    df_impacto.set_index("tipo_divergencia")["soma_diferenca_absoluta"]
                )
            else:
        ...
        if id_folha_selecionada:
            ...
            if (
                "predicao_risco_data" not in st.session_state
                or st.session_state.predicao_risco_data is None
                or st.session_state.predicao_risco_data.id_folha_processada
                != id_folha_selecionada
            ):
                try:
                    ...
                    dados_predicao_obj = PredicaoRiscoDashboardResponse(
                        **dados_predicao_api
                    )
                    st.session_state.predicao_risco_data = dados_predicao_obj
                except requests.exceptions.HTTPError as e_http_pred:
                    logger.error(
                        f"Erro HTTP ao buscar dados de predição: {e_http_pred.response.status_code} - {e_http_pred.response.text}"
                    )
                    if e_http_pred.response.status_code == 404:
                        st.info(
                            "Análise preditiva de riscos ainda não disponível para esta folha."
                        )
                    # O tratamento de 401 foi movido para cima.
                    else:
                        st.error(
                            f"Erro ao carregar análise preditiva ({e_http_pred.response.status_code}). Tente novamente."
                        )
                    st.session_state.predicao_risco_data = None
                except requests.exceptions.RequestException as e_req_pred:
                    logger.error(
                        f"Erro de conexão ao buscar dados de predição: {e_req_pred}"
                    )
                    st.error(
                        "Erro de conexão ao buscar análise preditiva. Verifique sua rede."
                    )
                    st.session_state.predicao_risco_data = None
                except Exception as e_gen_pred:
                    logger.error(
                        f"Erro inesperado ao processar dados de predição: {e_gen_pred}",
                        exc_info=True,
                    )
                    st.error(
                        "Ocorreu um erro inesperado ao carregar a análise preditiva."
                    )
                    st.session_state.predicao_risco_data = None
            else:
            ...
            if dados_predicao_obj:
                ...
                with col_score:
                    ...
                    if score is not None:
                        st.metric(
                            label="Score de Saúde da Folha (IA)", value=f"{score:.1f}%"
                        )
                        st.progress(int(score) / 100)
                    else:
                        st.metric(
                            label="Score de Saúde da Folha (IA)", value="Não Calculado"
                        )

                with col_risco_geral:
                    if (
                        dados_predicao_obj.classe_risco_geral
                        and dados_predicao_obj.classe_risco_geral != "INDISPONIVEL"
                    ):
                        st.metric(
                            label="Classificação de Risco Geral (IA)",
                            value=str(dados_predicao_obj.classe_risco_geral).upper(),
                        )
                    else:
                        st.metric(
                            label="Classificação de Risco Geral (IA)", value="N/A"
                        )

                if (
                    dados_predicao_obj.explicacao_geral_ia
                    and dados_predicao_obj.explicacao_geral_ia
                    != "Análise preditiva ainda não disponível ou não concluída para esta folha."
                ):
                    with st.expander(
                        "Ver Entendimento Geral da IA sobre os Riscos", expanded=False
                    ):
                ...
                if dados_predicao_obj.principais_riscos_previstos:
                    for risco in dados_predicao_obj.principais_riscos_previstos:
                        ...
                        cols_risco_detalhe[0].caption(
                            f"Severidade: **{risco.severidade_estimada or 'N/A'}**"
                        )
                        cols_risco_detalhe[1].caption(
                            f"Probabilidade: **{risco.probabilidade_estimada*100:.0f}%**"
                            if risco.probabilidade_estimada is not None
                            else "N/A"
                        )
                        cols_risco_detalhe[2].caption(
                            f"Fator Principal: *{risco.fator_principal or 'N/A'}*"
                        )

                        if container_risco.button(
                            "Ver Detalhes do Risco",
                            key=f"detail_risk_{risco.id_risco_detalhe}_{id_folha_selecionada}",
                        ):
                            st.session_state.detalhe_risco_selecionado_id = (
                                risco.id_risco_detalhe
                            )
                            st.session_state.tipo_detalhe_predicao = "risco_especifico"
                            st.session_state.id_folha_contexto_detalhe = (
                                id_folha_selecionada
                            )
                            st.rerun()

                        if container_risco.button(
                            "Analisar Risco com Consultor IA",
                            key=f"consult_risk_{risco.id_risco_detalhe}_{id_folha_selecionada}",
                        ):
                            st.session_state.risco_em_foco_para_consultor = (
                                risco.model_dump()
                            )
                            st.session_state.id_folha_ativa_contexto_chat = (
                                id_folha_selecionada
                            )
                            try:
                                ...
                                logger.warning(
                                    f"Falha ao usar st.switch_page para Consultor IA: {e_switch}"
                                )
                                st.page_link(
                                    "pages/2_🤖_Consultor_de_Riscos_IA.py",
                                    label="Analisar com Consultor IA",
                                    icon="🤖",
                                )
                else:
                    st.success(
                        "🎉 Nenhuma predição de risco significativa encontrada pela IA para esta folha!"
                    )

                if (
                    "tipo_detalhe_predicao" in st.session_state
                    and st.session_state.tipo_detalhe_predicao
                    and st.session_state.get("id_folha_contexto_detalhe")
                    == id_folha_selecionada
                ):

                    id_risco_foco_detalhe = st.session_state.get(
                        "detalhe_risco_selecionado_id"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Buscando detalhes da predição: {api_url_det} com params: {params_det}"
                        )

                        response_detalhes = requests.get(
                            api_url_det, headers=headers_det, params=params_det
                        )
                        if (
                            response_detalhes.status_code == 401
                        ):  # Adicionado tratamento com handle_api_error
                        ...
                        dados_detalhe_obj = DetalhePredicaoRiscoResponse(
                            **dados_detalhe_api
                        )

                    except requests.exceptions.HTTPError as e_http_det:
                        logger.error(
                            f"Erro HTTP ao buscar detalhes da predição: {e_http_det.response.status_code} - {e_http_det.response.text}"
                        )
                        # O tratamento de 401 foi movido para cima.
                        st.error(
                            f"Não foi possível carregar os detalhes da predição ({e_http_det.response.status_code})."
                        )
                    except requests.exceptions.RequestException as e_req_det:
                        logger.error(
                            f"Erro de conexão ao buscar detalhes da predição: {e_req_det}"
                        )
                        st.error("Erro de conexão ao buscar detalhes da predição.")
                    except Exception as e_gen_det:
                        logger.error(
                            f"Erro inesperado ao processar detalhes da predição: {e_gen_det}",
                            exc_info=True,
                        )
                        st.error(
                            "Ocorreu um erro inesperado ao carregar os detalhes da predição."
                        )
                    ...
                    if dados_detalhe_obj:
                        ...
                        if risco_selecionado_obj:
                            titulo_expander = (
                                f"🔍 Detalhes: {risco_selecionado_obj.descricao_risco}"
                            )
                        ...
                        with st.expander(titulo_expander, expanded=True):
                            ...
                            st.markdown(
                                "**Principais Fatores Técnicos Identificados:**"
                            )
                            if dados_detalhe_obj.fatores_contribuintes_tecnicos:
                                fatores_dumped = [
                                    f.model_dump()
                                    for f in dados_detalhe_obj.fatores_contribuintes_tecnicos
                                ]
                                df_fatores = pd.DataFrame(fatores_dumped)
                                if not df_fatores.empty:
                                    st.table(
                                        df_fatores[
                                            ["feature", "valor", "atribuicao_impacto"]
                                        ]
                                    )
                                else:
                                    st.caption(
                                        "Nenhum fator técnico principal detalhado."
                                    )
                            else:
                            ...
                            if dados_detalhe_obj.dados_suporte_visualizacao:
                                ...
                                for (
                                    visualizacao
                                ) in dados_detalhe_obj.dados_suporte_visualizacao:
                                    st.caption(
                                        visualizacao.titulo_grafico
                                        or "Visualização de Dados"
                                    )
                                    if (
                                        visualizacao.tipo_grafico == "SERIE_TEMPORAL"
                                        and isinstance(visualizacao.dados, list)
                                    ):
                                        ...
                                        try:
                                            ...
                                            if (
                                                not df_chart.empty
                                                and "x" in df_chart.columns
                                                and "y" in df_chart.columns
                                            ):
                                                st.line_chart(
                                                    df_chart.set_index("x")["y"]
                                                )
                                            elif (
                                                not df_chart.empty
                                            ):  # Tenta plotar a primeira coluna numérica se x,y não presentes
                                                st.line_chart(df_chart)
                                            else:
                                                st.caption(
                                                    "Dados de série temporal vazios ou malformados."
                                                )
                                        except Exception as e_chart:
                                            logger.error(
                                                f"Erro ao renderizar gráfico de série temporal: {e_chart}"
                                            )
                                            st.caption(
                                                "Não foi possível renderizar os dados da série temporal."
                                            )
                                    elif (
                                        visualizacao.tipo_grafico == "VALOR_SIMPLES"
                                        and isinstance(visualizacao.dados, dict)
                                    ):
                                        st.metric(
                                            label=visualizacao.dados.get(
                                                "label", "Valor"
                                            ),
                                            value=str(
                                                visualizacao.dados.get("valor", "N/A")
                                            ),
                                        )
                            ...
                            if st.button(
                                "Fechar Detalhes",
                                key=f"fechar_detalhe_{id_risco_foco_detalhe}_{id_folha_selecionada}",
                            ):
                                st.session_state.tipo_detalhe_predicao = None
                                st.session_state.detalhe_risco_selecionado_id = None
                                st.session_state.id_folha_contexto_detalhe = None
                ...
                if (
                    dados_predicao_obj
                    and dados_predicao_obj.score_saude_folha is not None
                ):
                    if st.button(
                        "Entender o Score de Saúde Geral",
                        key=f"detail_score_geral_{id_folha_selecionada}",
                    ):
        ...
        if st.button("Gerar e Baixar Relatório"):
            if tipo_relatorio_selecionado and id_folha_selecionada:
                ...
                try:
                    ...
                    response_rel = requests.get(
                        api_url_rel, headers=headers_rel, stream=True
                    )  # stream=True para download
                    if (
                        response_rel.status_code == 401
                    ):  # Adicionado tratamento com handle_api_error
                    ...
                    content_disposition = response_rel.headers.get(
                        "content-disposition"
                    )
                    ...
                    st.download_button(
                        ...
                        file_name=filename,
                        mime=response_rel.headers.get(
                            "content-type", "application/octet-stream"
                        ),  # Usar content-type do header se disponível
                    )
                    ...
                    logger.error(
                        f"Erro HTTP ao gerar relatório: {e_http_rel.response.status_code} - {e_http_rel.response.text}"
                    )
                    if e_http_rel.response.status_code == 401:
                        ...
                        st.error(
                            f"Erro ao gerar relatório ({e_http_rel.response.status_code})."
                        )
                    ...
                    logger.error(
                        f"Erro inesperado ao gerar relatório: {e_gen_rel}",
                        exc_info=True,
                    )
        ...
        if (
            id_cliente_atual and api_token and label_selecionada
        ):  # Se tudo estiver pronto para carregar
            st.info(
                "Clique em 'Carregar Dashboard' para visualizar os dados da folha selecionada."
            )
