...
def mostrar_pagina_gestao_cct():
    ...
    tab_upload, tab_listar, tab_monitor = st.tabs(
        ["📤 Upload de CCT", "🔎 Listar/Buscar CCTs", "⚙️ Monitoramento de Alertas"]
    )
    ...
    with tab_upload:
        ...
        with st.form("form_upload_cct", clear_on_submit=True):
            ...
            ids_afetados_default = (
                json.dumps([id_cliente_atual]) if id_cliente_atual else ""
            )
            ids_afetados = st.text_input(
                "IDs Clientes Afetados (JSON lista)", value=ids_afetados_default
            )
            ...
            if submit_btn:
                if not nome or not arquivo:
                    ...
                    form_data_payload = {
                        ...
                        "data_fim_vigencia_cct": (
                            data_fim.isoformat() if data_fim else None
                        ),
                        "sindicatos_laborais_json_str": (
                            json.dumps(
                                [
                                    s.strip()
                                    for s in sind_laborais.split("\n")
                                    if s.strip()
                                ]
                            )
                            if sind_laborais
                        ...
                        "sindicatos_patronais_json_str": (
                            json.dumps(
                                [
                                    s.strip()
                                    for s in sind_patronais.split("\n")
                                    if s.strip()
                                ]
                            )
                            if sind_patronais
                        ...
                        "ids_clientes_afetados_lista_str": ids_afetados
                        or json.dumps([id_cliente_atual]),
                    }
                    files = {"file": (arquivo.name, arquivo.getvalue(), arquivo.type)}

                    auth_headers_for_upload = (
                        get_auth_headers_cct()
                    ...
                    try:
                        logger.info(
                            f"Enviando CCT para {settings.API_BASE_URL}/ccts/upload. Payload: {form_data_payload.keys()}, File: {arquivo.name}"
                        )
                        resp = requests.post(
                        ...
                        data = resp.json()
                        st.success(
                            f"CCT enviada com sucesso! ID: {data.get('id_cct_documento')}"
                        )
                    except requests.exceptions.HTTPError as http_err:
                        logger.error(
                            f"Erro HTTP ao enviar CCT: {http_err.response.status_code} - {http_err.response.text}"
                        )
                        ...
                        try:
                            ...
                            pass
                        st.error(
                            f"Erro ao enviar CCT (HTTP {http_err.response.status_code}): {error_detail}"
                        )
                    except requests.exceptions.RequestException as e:
                        logger.error(
                            f"Erro de conexão ao enviar CCT: {e}", exc_info=True
                        )
                        st.error(f"Erro de conexão ao enviar CCT: {e}")
                    except json.JSONDecodeError:
                        logger.error(
                            f"Erro ao decodificar JSON da resposta do upload CCT: {resp.text if 'resp' in locals() else 'Resposta não disponível'}"
                        )
                        st.error(
                            "Erro ao processar a resposta do servidor (upload CCT)."
                        )
                    except Exception as e:
                        logger.error(
                            f"Erro inesperado no upload da CCT: {e}", exc_info=True
                        )
    ...
    with tab_listar:
        ...
        cliente_filter = col1.text_input(
            "ID Cliente Afetado", key="filter_cliente_cct", value=id_cliente_atual or ""
        )
        sindicato_filter = col2.text_input(
            "Filtro Sindicato (nome)", key="filter_sindicato_cct"
        )
        data_filter = col3.date_input(
            "Data Vigência em", value=None, key="filter_data_cct"
        )
        ...
        if st.button("Buscar CCTs", key="btn_buscar_ccts"):
            ...
            try:
                ...
                ccts = resp.json().get(
                    "ccts", []
                )  # Assumindo que a API retorna uma chave 'ccts' com a lista
            except requests.exceptions.HTTPError as http_err:
                logger.error(
                    f"Erro HTTP ao buscar CCTs: {http_err.response.status_code} - {http_err.response.text}"
                )
                error_detail = http_err.response.text
                try:
                    ...
                    pass
                st.error(
                    f"Erro ao buscar CCTs (HTTP {http_err.response.status_code}): {error_detail}"
                )
                ccts = []
            except requests.exceptions.RequestException as e:
                ...
                logger.error(
                    f"Erro ao decodificar JSON da busca de CCTs: {resp.text if 'resp' in locals() else 'Resposta não disponível'}"
                )
            ...
            if not ccts:
                ...
                for doc in ccts:
                    try:
                        sind_laborais_list = (
                            json.loads(doc.get("sindicatos_laborais_json_str", "[]"))
                            if doc.get("sindicatos_laborais_json_str")
                            else []
                        )
                        sind_patronais_list = (
                            json.loads(doc.get("sindicatos_patronais_json_str", "[]"))
                            if doc.get("sindicatos_patronais_json_str")
                            else []
                        )
                    except json.JSONDecodeError:
                    ...
                    table_data.append(
                        {
                            ...
                            "Download": (
                                f"[Baixar]({doc.get('gcs_uri_documento')})"
                                if doc.get("gcs_uri_documento")
                ...
                )  # Usar st.dataframe para melhor formatação e interatividade
    ...
    with tab_monitor:
        ...
        try:
            response = requests.get(
                f"{settings.API_BASE_URL}/ccts/alerts",
                params=params,
                headers=get_auth_headers_cct(),
            )
            ...
            alerts = response.json().get(
                "alerts", []
            )  # Assumindo que a API retorna uma chave 'alerts'
        except requests.exceptions.HTTPError as http_err:
            logger.error(
                f"Erro HTTP ao buscar alertas CCT: {http_err.response.status_code} - {http_err.response.text}"
            )
            error_detail = http_err.response.text
            try:
                error_detail = http_err.response.json().get("detail", error_detail)
                ...
                pass
            st.error(
                f"Erro ao buscar alertas (HTTP {http_err.response.status_code}): {error_detail}"
            )
            alerts = []
        except requests.exceptions.RequestException as e:
            ...
            logger.error(
                f"Erro ao decodificar JSON da busca de alertas CCT: {response.text if 'response' in locals() else 'Resposta não disponível'}"
            )
        ...
        if not alerts:
            ...
            for alert in alerts:
                ...
                with st.expander(exp_title):
                    st.markdown(
                        f"**Registro MTE:** {alert.get('numero_registro_mte_detectado', '-')}"
                    )
                    st.markdown(
                        f"**Vigência Início:** {alert.get('vigencia_inicio_detectada', '-')}"
                    )
                    st.markdown(
                        f"**Sindicato(s) Partes:** {alert.get('sindicatos_partes_detectados', '-')}"
                    )
                    st.markdown(
                        f"**Link Fonte:** {alert.get('link_fonte_cct_detectada', '-')}"
                    )
                    ...
                    with col_status:
                        new_status = st.selectbox(
                            ...
                            options=[
                                ...
                                "CCT_IMPORTADA",
                                "DESCARTADO",
                            ],  # Adicionar todos os status válidos
                            index=(
                                [
                                    "NOVO",
                                    "EM_REVISAO_ADMIN",
                                    "CCT_IMPORTADA",
                                    "DESCARTADO",
                                ].index(alert.get("status_alerta"))
                                if alert.get("status_alerta")
                                in [
                                    "NOVO",
                                    "EM_REVISAO_ADMIN",
                                    "CCT_IMPORTADA",
                    ...
                    if st.button(
                        "Atualizar Alerta",
                        key=f"btn_update_alert_{alert.get('id_alerta_cct')}",
                    ):
                        update_payload = {
                            "status_alerta": new_status,
                            "notas_admin": notes,
                            "id_cliente": id_cliente_atual,
                        }
                        logger.info(
                            f"Atualizando alerta CCT ID {alert.get('id_alerta_cct')} com payload: {update_payload}"
                        )
                        try:
                            ...
                            st.success(
                                f"Alerta {alert.get('id_alerta_cct')} atualizado com sucesso."
                            )
                            st.rerun()
                        except requests.exceptions.HTTPError as http_err_put:
                            logger.error(
                                f"Erro HTTP ao atualizar alerta CCT: {http_err_put.response.status_code} - {http_err_put.response.text}"
                            )
                            error_detail_put = http_err_put.response.text
                            try:
                                error_detail_put = http_err_put.response.json().get(
                                    "detail", error_detail_put
                                )
                            except json.JSONDecodeError:
                                pass
                            st.error(
                                f"Erro ao atualizar alerta {alert.get('id_alerta_cct')} (HTTP {http_err_put.response.status_code}): {error_detail_put}"
                            )
                        except requests.exceptions.RequestException as e_put:
                            logger.error(
                                f"Erro de conexão ao atualizar alerta CCT: {e_put}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro ao atualizar alerta {alert.get('id_alerta_cct')}: {e_put}"
                            )
                        except Exception as e_gen_put:
                            logger.error(
                                f"Erro inesperado ao atualizar alerta CCT: {e_gen_put}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro inesperado ao processar a atualização do alerta: {e_gen_put}"
                            )
...
if __name__ == "__main__":
    ...
    ):  # user_info is used by display_user_info_sidebar
        st.session_state.user_info = {
            "name": "Usuário Teste CCT",
            "username": "testuser_cct",
        }
