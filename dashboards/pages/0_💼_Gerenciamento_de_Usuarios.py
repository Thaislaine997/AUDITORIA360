...
st.set_page_config(
    page_title="Gerenciamento de Usuários - AUDITORIA360",
    layout="wide"
)
...
# --- Path Setup ---
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))  # Ajustado para subdiretório pages
...
def mostrar_pagina_gerenciamento_usuarios():
    ...
    if "admin" in user_roles:
        ...
        if "users_data" not in st.session_state or st.session_state["users_data"] is None:
            try:
                headers = {"Authorization": f"Bearer {st.session_state.get('api_token')}"}
                response = requests.get(
                    f"{settings.API_BASE_URL}/api/users/all",
                    headers=headers
                )
                ...
                if response.status_code == 200:
                    ...
                    st.dataframe(
                        users_df[["id", "nome", "email", "cliente_id", "roles", "ativo"]],
                    ...
                    with st.expander("Adicionar Novo Usuário"):
                        with st.form("novo_usuario_form"):
                            ...
                            with col2:
                                ...
                                roles_options = ["admin", "auditor", "cliente", "consultor"]
                            ...
                            if submit_button:
                                ...
                                if not novo_nome or not novo_email or not nova_senha:
                                    ...
                                    new_user_data = {
                                        ...
                                        "email": novo_email,
                                        "senha": nova_senha,
                                        "cliente_id": novo_cliente_id if novo_cliente_id else None,
                                        "roles": novas_roles,
                                        "ativo": True
                                    }
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if client_users:
                        ...
                        st.info("Não há outros usuários registrados para sua organização.")
                else:
                    st.error("Não foi possível carregar os usuários da sua organização.")
            except Exception as e:
                st.error(f"Erro ao conectar com a API: {str(e)}")
        else:
