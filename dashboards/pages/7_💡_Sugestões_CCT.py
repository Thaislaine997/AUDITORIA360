...
st.set_page_config(
    page_title="Sugestões de Impacto CCT - AUDITORIA360",
    layout="wide",
...
def mostrar_pagina_sugestoes_cct():  # Renomeado para seguir padrão
    ...
    id_cct_doc_filter = st.text_input(
        "ID do Documento CCT (opcional, para filtrar)", key="sug_id_cct_doc"
    )
    ...
    if st.button("Buscar Sugestões Pendentes", key="btn_buscar_sugestoes_cct"):
        ...
        with st.spinner("Buscando sugestões..."):
            ...
            try:
                ...
                st.session_state.sugestoes_cct_data = response.json().get(
                    "sugestoes", []
                )  # Assumindo que a API retorna {"sugestoes": [...]}
                logger.info(
                    f"Sugestões encontradas: {len(st.session_state.sugestoes_cct_data)}"
                )
            except requests.exceptions.HTTPError as http_err:
                logger.error(
                    f"Erro HTTP ao buscar sugestões CCT: {http_err.response.status_code} - {http_err.response.text}"
                )
                ...
                try:
                    ...
                    pass
                st.error(
                    f"Erro ao buscar sugestões (HTTP {http_err.response.status_code}): {error_detail}"
                )
            except requests.exceptions.RequestException as e:
                logger.error(
                    f"Erro de conexão ao buscar sugestões CCT: {e}", exc_info=True
                )
                st.error(f"Erro de conexão ao buscar sugestões: {e}")
            except json.JSONDecodeError:
                logger.error(
                    f"Erro ao decodificar JSON da busca de sugestões CCT: {response.text if 'response' in locals() else 'Resposta não disponível'}"
                )
                st.error("Erro ao processar a resposta do servidor (sugestões CCT).")
            except Exception as e:
                logger.error(
                    f"Erro inesperado na busca de sugestões CCT: {e}", exc_info=True
                )
    ...
    if not sugestoes_para_exibir:
        st.info(
            "Nenhuma sugestão pendente encontrada para os filtros aplicados ou busca ainda não realizada."
        )
    else:
        for sugestao in sugestoes_para_exibir:
            sugestao_id = sugestao.get("id_sugestao_impacto", "N/A")
            with st.expander(
                f"Sugestão ID {sugestao_id} - Tipo: {sugestao.get('tipo_sugestao', 'N/A')}"
            ):
                # ...existing code...
                # (Conteúdo do expander permanece o mesmo, apenas ajustando a obtenção do username abaixo)
                st.markdown(
                    f"**Cláusula Base:** {sugestao.get('texto_clausula_cct_base', 'N/A')}"
                )
                st.markdown(
                    f"**Justificativa IA:** {sugestao.get('justificativa_sugestao_ia', 'N/A')}"
                )
                st.markdown(
                    f"**Data de Vigência Sugerida:** {sugestao.get('data_inicio_vigencia_sugerida', 'N/A')}"
                )
                st.markdown(
                    f"**Status:** {sugestao.get('status_sugestao', 'PENDENTE')}"
                )

                dados_editados = {}
                if sugestao.get("tipo_sugestao") == "ALTERACAO_RUBRICA_CLIENTE":
                    st.markdown(
                        f"**Rubrica Afetada:** {sugestao.get('codigo_rubrica_cliente_existente', 'N/A')}"
                    )
                    try:
                        alteracoes_str = sugestao.get(
                            "json_alteracoes_sugeridas_rubrica"
                        )
                        alteracoes = (
                            json.loads(alteracoes_str) if alteracoes_str else []
                        )
                        ...
                        for i, alt_item in enumerate(alteracoes):
                            if isinstance(
                                alt_item, dict
                            ):  # Processar apenas se for um dicionário
                                campo_alt = alt_item.get(
                                    "campo_a_alterar", f"campo_desconhecido_{i}"
                                )
                                ...
                                logger.warning(
                                    f"Item em json_alteracoes_sugeridas_rubrica não é um dict: {alt_item} para sugestão {sugestao_id}"
                                )
                                novas_alteracoes_editadas.append(
                                    alt_item
                                )  # Manter se não for dict, ou tratar conforme necessário

                        dados_editados["json_alteracoes_sugeridas_rubrica"] = (
                            json.dumps(novas_alteracoes_editadas)
                        )
                    except json.JSONDecodeError:
                        logger.error(
                            f"Erro ao decodificar JSON de alterações para sugestão {sugestao_id}: {sugestao.get('json_alteracoes_sugeridas_rubrica')}"
                        )
                        st.error("Erro ao decodificar JSON de alterações sugeridas.")
                        dados_editados["json_alteracoes_sugeridas_rubrica"] = (
                            sugestao.get("json_alteracoes_sugeridas_rubrica", "[]")
                        )

                elif sugestao.get("tipo_sugestao") == "NOVA_RUBRICA_CLIENTE":
                    dados_editados["codigo_sugerido_nova_rubrica"] = st.text_input(
                        "Código Nova Rubrica",
                        value=sugestao.get("codigo_sugerido_nova_rubrica", ""),
                        key=f"cod_nr_{sugestao_id}",
                    )
                    dados_editados["descricao_sugerida_nova_rubrica"] = st.text_input(
                        "Descrição Nova Rubrica",
                        value=sugestao.get("descricao_sugerida_nova_rubrica", ""),
                        key=f"desc_nr_{sugestao_id}",
                    )
                    dados_editados["tipo_sugerido_nova_rubrica"] = st.text_input(
                        "Tipo Nova Rubrica",
                        value=sugestao.get("tipo_sugerido_nova_rubrica", ""),
                        key=f"tipo_nr_{sugestao_id}",
                    )
                    dados_editados["natureza_esocial_sugerida"] = st.text_input(
                        "Natureza eSocial",
                        value=sugestao.get("natureza_esocial_sugerida", ""),
                        key=f"nat_esocial_nr_{sugestao_id}",
                    )
                    try:
                        incidencias_str = sugestao.get(
                            "json_sugestao_incidencias_completa_nova_rubrica"
                        )
                        incidencias = (
                            json.loads(incidencias_str) if incidencias_str else {}
                        )
                        if not isinstance(incidencias, dict):
                            incidencias = {}  # Garantir que é um dict
                        ...
                        for k, v_sug in incidencias.items():
                            valor_editado = st.text_input(
                                ...
                                value=str(
                                    v_sug
                                ),  # Garantir que o valor é string para o input
                                key=f"{sugestao_id}_incidencia_{k}",
                            )
                            novas_incidencias_editadas[k] = valor_editado
                        dados_editados[
                            "json_sugestao_incidencias_completa_nova_rubrica"
                        ] = json.dumps(novas_incidencias_editadas)
                    except json.JSONDecodeError:
                        logger.error(
                            f"Erro ao decodificar JSON de incidências para sugestão {sugestao_id}: {sugestao.get('json_sugestao_incidencias_completa_nova_rubrica')}"
                        )
                ...
                elif sugestao.get("tipo_sugestao", "").startswith(
                    "ATUALIZACAO_PARAMETRO_LEGAL"
                ):
                    dados_editados["novo_valor_sugerido_parametro"] = st.text_input(
                        "Novo Valor Sugerido",
                        value=sugestao.get("novo_valor_sugerido_parametro", ""),
                        key=f"val_param_{sugestao_id}",
                    )

                notas = st.text_area(
                    "Notas de Revisão",
                    key=f"notas_{sugestao_id}",
                    value=sugestao.get("notas_revisao_usuario", ""),
                )  # Carregar notas existentes

                col1, col2 = st.columns(2)

                # Obter username de forma padronizada
                username = st.session_state.get("user_info", {}).get(
                    "username", "usuario_desconhecido"
                )

                auth_hdrs_process = (
                    get_auth_headers_sugestoes()
                ...
                with col1:
                    if st.button("Aprovar e Aplicar", key=f"aprovar_{sugestao_id}"):
                        payload = {
                            ...
                            "dados_sugestao_atualizados_json": (
                                json.dumps(dados_editados) if dados_editados else None
                            ),
                        }
                        logger.info(
                            f"Aprovando sugestão {sugestao_id}. Payload: {payload}"
                        )
                        try:
                            response_process = requests.post(
                                api_url_process, headers=auth_hdrs_process, json=payload
                            )
                            ...
                            st.success(
                                f"Sugestão {sugestao_id} aprovada e aplicada com sucesso!"
                            )
                            logger.info(f"Sugestão {sugestao_id} aprovada.")
                            st.session_state.sugestoes_cct_data = (
                                []
                            )  # Limpar para forçar recarga na próxima interação
                            st.rerun()
                        except requests.exceptions.HTTPError as http_err_proc:
                            logger.error(
                                f"Erro HTTP ao aprovar sugestão {sugestao_id}: {http_err_proc.response.status_code} - {http_err_proc.response.text}"
                            )
                            error_detail_proc = http_err_proc.response.text
                            try:
                                error_detail_proc = http_err_proc.response.json().get(
                                    "detail", error_detail_proc
                                )
                            except json.JSONDecodeError:
                                pass
                            st.error(
                                f"Erro ao aprovar sugestão {sugestao_id} (HTTP {http_err_proc.response.status_code}): {error_detail_proc}"
                            )
                        except requests.exceptions.RequestException as e_proc:
                            logger.error(
                                f"Erro de conexão ao aprovar sugestão {sugestao_id}: {e_proc}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro de conexão ao aprovar sugestão {sugestao_id}: {e_proc}"
                            ...
                            logger.error(
                                f"Erro inesperado ao processar aprovação da sugestão {sugestao_id}: {e_gen_proc}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro inesperado ao processar aprovação: {e_gen_proc}"
                            )
                ...
                with col2:
                    if st.button("Rejeitar", key=f"rejeitar_{sugestao_id}"):
                        ...
                        logger.info(
                            f"Rejeitando sugestão {sugestao_id}. Payload: {payload}"
                        )
                        try:
                            response_process = requests.post(
                                api_url_process, headers=auth_hdrs_process, json=payload
                            ...
                            st.session_state.sugestoes_cct_data = (
                                []
                            )  # Limpar para forçar recarga
                            st.rerun()
                        except requests.exceptions.HTTPError as http_err_rej:
                            logger.error(
                                f"Erro HTTP ao rejeitar sugestão {sugestao_id}: {http_err_rej.response.status_code} - {http_err_rej.response.text}"
                            )
                            error_detail_rej = http_err_rej.response.text
                            try:
                                error_detail_rej = http_err_rej.response.json().get(
                                    "detail", error_detail_rej
                                )
                            except json.JSONDecodeError:
                                pass
                            st.error(
                                f"Erro ao rejeitar sugestão {sugestao_id} (HTTP {http_err_rej.response.status_code}): {error_detail_rej}"
                            )
                        except requests.exceptions.RequestException as e_rej:
                            logger.error(
                                f"Erro de conexão ao rejeitar sugestão {sugestao_id}: {e_rej}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro ao rejeitar sugestão {sugestao_id}: {e_rej}"
                            )
                        except Exception as e_gen_rej:
                            logger.error(
                                f"Erro inesperado ao processar rejeição da sugestão {sugestao_id}: {e_gen_rej}",
                                exc_info=True,
                            )
                            st.error(
                                f"Erro inesperado ao processar rejeição: {e_gen_rej}"
                            )
    ...
    if token:
        ...
        sugestoes_filtradas = [
            s for s in sugestoes if filtro.lower() in s.get("descricao", "").lower()
        ]
...
if __name__ == "__main__":
    ...
    if "user_info" not in st.session_state:
        st.session_state.user_info = {
            "name": "Usuário Sugestão Teste",
            "username": "sugestao_user",
        }
