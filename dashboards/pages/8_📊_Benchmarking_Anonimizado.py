...
# --- Path Setup ---
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..')) # Path for pages subfolder
...
from src.frontend.components.layout import header, footer
import src.frontend.components.widget_async_loader as async_loader
...
st.set_page_config(
    page_title="Benchmarking Anonimizado - Auditoria360",
    page_icon="📊",
    layout="wide"
)
...
def carregar_dados_benchmark(filtros=None):
    ...
    try:
        ...
        if resposta.status_code == 200:
            ...
            st.warning("É necessário dar consentimento para acessar os dados de benchmark.")
            return None
        else:
...
def plotar_comparacao_setores(dados, kpi_nome, kpi_titulo, unidade_medida):
    ...
    # Agrupar por setor
    df_plot = df[["setor_empresa", "valor_medio", "mediana"]].groupby("setor_empresa").agg({
        "valor_medio": "mean",
        "mediana": "mean"
    }).reset_index()
...
def plotar_comparacao_portes(dados, kpi_nome, kpi_titulo, unidade_medida):
    ...
    # Agrupar por porte
    df_plot = df[["porte_empresa", "valor_medio", "mediana"]].groupby("porte_empresa").agg({
        "valor_medio": "mean",
        "mediana": "mean"
    }).reset_index()
...
def plotar_tendencia_tempo(dados, kpi_nome, kpi_titulo, unidade_medida):
    ...
    # Agrupar por período
    df_plot = df[["periodo_referencia", "valor_medio", "mediana"]].groupby("periodo_referencia").agg({
        "valor_medio": "mean",
        "mediana": "mean"
    }).reset_index()
...
def exibir_secao_consentimento():
    ...
    status_atual = consentimento.get("status_consentimento", False) if consentimento else False
...
def exibir_graficos_benchmark(dados, kpi_selecionado):
    ...
    kpi_categoria = kpi_dados[0]["kpi_categoria"]
    ...
    with col1:
        st.plotly_chart(
            plotar_comparacao_setores(dados, kpi_selecionado, kpi_titulo, unidade_medida),
            use_container_width=True
        )
        
    with col2:
        st.plotly_chart(
            plotar_comparacao_portes(dados, kpi_selecionado, kpi_titulo, unidade_medida),
            use_container_width=True
        )
    ...
    with st.expander("Ver dados detalhados"):
        ...
        colunas = [
            "periodo_referencia", "setor_empresa", "porte_empresa", "regiao",
            "valor_medio", "mediana", "percentil_25", "percentil_75", 
            "valor_minimo", "valor_maximo", "quantidade_empresas"
        ]
...
def carregar_e_exibir_dados():
        ...
        with col3:
            ...
            if opcoes_kpi:
                kpi_selecionado = st.selectbox(
                    ...
                    format_func=lambda x: next((k["kpi_titulo"] for k in kpis if k["kpi_nome"] == x), x)
                )
            else:
    ...
    if not dados_benchmark:
        st.info("Não há dados disponíveis para os filtros selecionados ou é necessário consentimento para acessar os dados.")
        return
...
def main():
    ...
    tab_dashboard, tab_config = st.tabs(["Dashboard de Benchmarking", "Configurações de Participação"])
