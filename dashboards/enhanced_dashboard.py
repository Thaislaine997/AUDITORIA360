...
class DashboardData:
    ...
    def get_trend_data():
        ...
        return pd.DataFrame({
            'Data': dates,
            'Auditorias_Concluidas': [45 + i*2 + (i%7)*5 for i in range(len(dates))],
            'Anomalias': [3 + (i%5) for i in range(len(dates))],
            'Score_Conformidade': [90 + (i%10) + (i*0.1) for i in range(len(dates))],
...
def render_sidebar():
    ...
    department = st.sidebar.selectbox(
        "Departamento",
        ["Todos", "RH", "Financeiro", "Operacional", "TI"]
    )
...
def render_compliance_analysis():
    ...
    with col1:
        ...
        fig_pie.update_layout(
            showlegend=True,
            height=400,
            title_text="Níveis de Conformidade"
        )
...
def render_recent_activity():
    ...
    styled_data = recent_data.style.applymap(
        style_status,
        subset=['Status']
    )
