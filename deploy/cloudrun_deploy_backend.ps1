# ================================================================
# Script: cloudrun_deploy_backend.ps1
# Descrição: Script para build e deploy do backend AUDITORIA360 no Google Cloud Run
# Uso: .\cloudrun_deploy_backend.ps1 [parâmetros]
...
function Test-ProjectStructure {
    Write-InfoLog "Verificando estrutura do projeto..."
    ...
    Write-SuccessLog "Estrutura do projeto validada"
}
...
function Invoke-DockerBuild {
    ...
    Write-InfoLog "Iniciando build da imagem Docker..."
    Write-InfoLog "Imagem: $ImageName:$ImageTag"
    
    try {
        # Executar build
        $buildCommand = "gcloud builds submit --tag ${ImageName}:${ImageTag}"
...
function Get-EnvironmentVariables {
    Write-InfoLog "Configurando variáveis de ambiente..."
    
    $envVars = @{
        "GCP_PROJECT_ID" = $ProjectId
        "BQ_DATASET_ID" = "auditoria_folha_dataset"
        "CCT_TEXT_BUCKET_NAME" = "auditoria360-ccts-textos-extraidos"
        "CCT_EXTRATOR_PROCESSOR_ID" = "SEU_PROCESSOR_ID_DOCAI"
...
function Invoke-CloudRunDeploy {
    Write-InfoLog "Iniciando deploy no Cloud Run..."
...
function Test-ServiceHealth {
    Write-InfoLog "Verificando status do serviço..."
    ...
    try {
        ...
        Write-SuccessLog "Serviço disponível em: $serviceUrl"
        
        # Teste básico de conectividade (opcional)
        try {
            $healthCheck = Invoke-WebRequest -Uri "$serviceUrl/health" -TimeoutSec 30 -UseBasicParsing
            if ($healthCheck.StatusCode -eq 200) {
                Write-SuccessLog "Health check passou: HTTP $($healthCheck.StatusCode)"
            }
            else {
                Write-WarningLog "Health check retornou: HTTP $($healthCheck.StatusCode)"
            }
        }
        catch {
            Write-WarningLog "Health check não disponível ou falhou"
        }
    }
    catch {
...
function Show-DeploymentInfo {
    Write-InfoLog "=== INFORMAÇÕES DO DEPLOY ==="
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($serviceUrl)) {
            Write-InfoLog "URL do serviço: $serviceUrl"
            Write-InfoLog "Documentação da API: $serviceUrl/docs"
        }
    }
    catch {
        Write-WarningLog "Não foi possível obter URL do serviço"
...
function Invoke-Main {
    try {
        Write-InfoLog "=== AUDITORIA360 - DEPLOY DO BACKEND NO CLOUD RUN ==="
        Write-InfoLog "Iniciando deploy do backend..."
        ...
        Write-SuccessLog "Deploy do backend concluído com sucesso!"
    }
    catch {
