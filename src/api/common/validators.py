    ...
    def validate_page_size(cls, v):
        if v > 100:
            raise validation_error(
                "Page size cannot exceed 100", field="page_size", value=v
            )
        return v
    ...
    def validate_date_range(cls, v, info):
        if v and "start_date" in info.data and info.data["start_date"]:
            if v < info.data["start_date"]:
                raise validation_error(
                    "End date must be after start date", field="end_date", value=v
                )
        return v
...
class SortParams(BaseValidationModel):
    ...
    def validate_sort_order(cls, v):
        if v not in ["asc", "desc"]:
            raise validation_error(
                "Sort order must be 'asc' or 'desc'", field="sort_order", value=v
            )
        return v
...
class StandardListParams(PaginationParams, DateRangeParams, SortParams):
    ...
    def validate_search(cls, v):
        if v is not None and len(v.strip()) < 2:
            raise validation_error(
                "Search term must be at least 2 characters", field="search", value=v
            )
...
def validate_phone(phone: str) -> bool:
    ...
    return len(phone) in [10, 11] and phone[0] in [
        "1",
        "2",
        "3",
        "4",
...
def validate_date_range(
    ...
    if start_date and end_date and end_date < start_date:
        raise validation_error("End date must be after start date", field="end_date")
...
def validate_pagination_params(page: int, page_size: int) -> None:
    ...
    if page_size < 1 or page_size > 100:
        raise validation_error(
            "Page size must be between 1 and 100", field="page_size", value=page_size
        )
