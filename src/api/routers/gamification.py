...
async def get_gamification_profile(
    ...
    try:
        ...
        user_achievements_query = (
            ...
            .filter(user_achievements.c.user_id == current_user.id)
            .all()
        )
        ...
        recent_xp = (
            ...
            .order_by(desc(XPHistory.earned_at))
            .limit(10)
            .all()
        )
        ...
        if current_user.onboarding_status == "in_progress":
            current_mission = (
                ...
                .filter(OnboardingMission.id == current_user.current_mission_id)
                .first()
            )
        ...
        if reason == "mission_completed":
            ...
            mission_achievements = (
                ...
                .filter(
                    ...
                    Achievement.criteria_target
                    <= current_user.total_missions_completed,
                    Achievement.is_active == True,
                )
                .all()
            )
...
async def complete_mission(
    ...
    try:
        ...
        mission = (
            db.query(OnboardingMission)
            .filter(
                OnboardingMission.id == mission_id, OnboardingMission.is_active == True
            )
            .first()
        )
        ...
        if (
            mission.profile_target != "all"
            and mission.profile_target != current_user.user_profile
        ):
            raise HTTPException(
        ...
        next_mission = (
            ...
            .filter(
                ...
                OnboardingMission.profile_target.in_(
                    ["all", current_user.user_profile]
                ),
                OnboardingMission.is_active == True,
            )
            .order_by(OnboardingMission.order_sequence)
            .first()
        )
...
async def get_team_leaderboard(
    ...
    try:
        ...
        team_members = (
            db.query(User)
            .filter(User.department == current_user.department, User.status == "active")
...
async def update_skill_progress(
    ...
    try:
        ...
        if (
            not skill_progress.is_unlocked
            and skill_progress.current_progress >= skill.required_actions
        ):
...
async def get_available_achievements(db: Session = Depends(get_db)):
