...
async def trigger_daily_reports(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    """Trigger daily report generation (Vercel Cron)"""
    if not run_scheduled_reports:
        raise HTTPException(status_code=503, detail="Scheduled reports module not available")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger daily reports: {str(e)}")

@router.post("/reports/weekly")
async def trigger_weekly_reports(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger weekly reports: {str(e)}")

@router.post("/reports/monthly")
async def trigger_monthly_reports(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger monthly reports: {str(e)}")

@router.post("/backup/daily")
async def trigger_daily_backup(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    """Trigger daily backup routine (Vercel Cron)"""
    if not run_backup_routine:
        raise HTTPException(status_code=503, detail="Backup routine module not available")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger daily backup: {str(e)}")

@router.post("/payroll/process")
async def trigger_payroll_processing(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    """Trigger payroll processing automation (Vercel Cron - weekdays)"""
    if not run_payroll_automation:
        raise HTTPException(status_code=503, detail="Payroll automation module not available")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger payroll processing: {str(e)}")

@router.post("/comunicados/generate")
async def trigger_comunicados_generation(background_tasks: BackgroundTasks, auth: bool = Depends(verify_cron_auth)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger comunicados generation: {str(e)}")
...
async def test_automation_module(module: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to test {module} module: {str(e)}")
