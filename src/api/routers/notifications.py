...
async def twilio_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = notification_service.process_incoming_message(
            webhook_data=webhook_data, provider_name="twilio"
        )
...
async def get_gateway_status(current_user: MockUser = Depends(get_current_user_mock)):
    """
...
async def configure_communication_provider(
    config_data: Dict[str, Any], current_user: MockUser = Depends(get_current_user_mock)
):
    """
    ...
    if provider_name == "twilio":
        ...
        if not config.get("from_sms") and not config.get("from_whatsapp"):
            validation_result["warnings"].append(
                "At least one phone number (SMS or WhatsApp) should be configured"
            )
