...
async def receive_intention(intention: UserIntention):
    ...
    try:
        logger.info(f"🧠 Neuro-Symbolic: Received intention {intention.type} for {intention.target} "
        ...
        logger.info(f"🚀 Neuro-Symbolic: Pre-loaded data for {intention.target} "
        ...
        return IntentionResponse(
            success=False,
            message=f"Failed to process intention: {str(e)}"
        )
...
async def get_intention_analytics():
    ...
    return {
        ...
        "processed_intentions": len([i for i in intention_cache.values() if i["processed"]]),
...
def get_most_common_targets() -> List[Dict[str, Any]]:
    ...
    return [{"target": k, "count": v} for k, v in sorted(targets.items(), key=lambda x: x[1], reverse=True)]
...
def get_average_confidence() -> float:
    ...
    confidences = [entry["intention"]["confidence"] for entry in intention_cache.values()]
...
async def clear_intention_cache():
    ...
    return {
        "success": True,
        "message": "Intention cache cleared"
    }