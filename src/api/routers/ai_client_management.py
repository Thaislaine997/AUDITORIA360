...
async def get_clients_with_ai_insights(
    ...
    try:
        clients_query = (
            db.query(Client)
            .filter(Client.created_by_user_id == current_user.id)
            .offset(skip)
            .limit(limit)
        )
        ...
        for client in clients_query.all():
            ...
            if include_risk_analysis:
                ...
                latest_analysis = (
                    db.query(ChurnRiskAnalysis)
                    .filter(ChurnRiskAnalysis.client_id == client.id)
                    .order_by(desc(ChurnRiskAnalysis.analysis_date))
                    .first()
                )
...
async def get_client_risk_analysis(
    ...
    try:
        ...
        analyses = (
            db.query(ChurnRiskAnalysis)
            .filter(ChurnRiskAnalysis.client_id == client_id)
            .order_by(desc(ChurnRiskAnalysis.analysis_date))
            .limit(10)
            .all()
        )
...
async def analyze_churn_risk(
    ...
    try:
        ...
        background_tasks.add_task(
            perform_churn_analysis, client_id, current_user.id, db
        )
...
async def get_configuration_suggestions(
    ...
    try:
        ...
        if (
            client.tax_regime == "Simples Nacional"
            and client.annual_revenue
            and client.annual_revenue > 1000000
        ):
            suggestions.append(
                {
                    "type": "document",
            ...
            if client.tax_regime == "Simples Nacional":
                ...
                for doc in required_docs:
                    if doc not in documents:
                        warnings.append(
                            f"Documento '{doc}' recomendado para {client.tax_regime}"
                        )
            ...
            for recipient in configuration_data["recipients"]:
                if "@" not in recipient.get("email", ""):
                    validation_errors.append(
                        f"Email inválido: {recipient.get('email')}"
                    )
        ...
        return success_response(
            {
                ...
                "recommendation": (
                    ...
                    if is_successful
                    else "Corrija os erros antes de aplicar"
                ),
            }
        )
