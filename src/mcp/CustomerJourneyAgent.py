...
class CustomerJourneyAgent:
    ...
    async def _analyze_data_volume_patterns(self) -> List[BehaviorPattern]:
        ...
        high_volume_users = [
            {"user_id": 1005, "data_volume_gb": 45, "processing_frequency": "daily"},
            {
                "user_id": 1006,
                "data_volume_gb": 78,
                "processing_frequency": "twice_daily",
            },
        ]
    ...
    def generate_upsell_opportunities(
    ...
    async def trigger_contextual_notifications(
        self, opportunities: List[UpsellOpportunity]
    ) -> Dict[str, int]:
        """
        ...
        for opportunity in opportunities:
            try:
                ...
                logger.info(
                    f"📧 Triggered notification for user {opportunity.user_id}: {opportunity.recommended_feature}"
                )
                ...
                logger.error(
                    f"Failed to send notification for user {opportunity.user_id}: {e}"
                )
                results["errors"] += 1
    ...
    async def run_daily_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            notification_results = await self.trigger_contextual_notifications(
                opportunities
            )
            ...
            results = {
                ...
                "summary": {
                    "total_potential_value": sum(
                        opp.estimated_value for opp in opportunities
                    ),
                    "avg_confidence": (
                        sum(opp.confidence_score for opp in opportunities)
                        / len(opportunities)
                        if opportunities
                        else 0
                    ),
            ...
            logger.info(
                f"✅ Daily analysis complete. {len(opportunities)} opportunities identified with ${results['summary']['total_potential_value']:.2f} potential value"
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(
            f"- Total potential value: ${results['summary']['total_potential_value']:.2f}"
        )
        print(
            f"- Most recommended feature: {results['summary']['most_recommended_feature']}"
        )
