...
from .protocol import (
    AgentRole, ConsensusLevel, ConsensusVote, ConsensusResult, 
    MCPConsensusRequest, ErrorCode
)
...
class ConsensusAgent:
    ...
    async def evaluate_insight(self, insight_data: Dict, context: Dict = None) -> ConsensusVote:
        ...
        vote, confidence, reasoning = self._role_specific_evaluation(insight_data, context)
    ...
    def _role_specific_evaluation(self, insight_data: Dict, context: Dict = None) -> tuple:
        """Role-specific evaluation logic"""
    ...
    def _security_evaluation(self, insight_data: Dict, context: Dict = None) -> tuple:
        ...
        if security_score < 0.3:
            return False, 0.9, "Security score too low - potential security threat detected"
        elif security_score < 0.6:
            return False, 0.7, "Security concerns identified - requires additional validation"
        else:
    ...
    def _risk_evaluation(self, insight_data: Dict, context: Dict = None) -> tuple:
        ...
        if risk_level == "critical":
            return False, 0.95, "Critical risk level detected - requires immediate mitigation"
    ...
    def _compliance_evaluation(self, insight_data: Dict, context: Dict = None) -> tuple:
        ...
        if any("violation" in flag.lower() for flag in compliance_flags):
            return False, 0.95, f"Compliance violations detected: {', '.join(compliance_flags)}"
        elif compliance_flags:
            return True, 0.7, f"Compliance flags noted but acceptable: {', '.join(compliance_flags)}"
        else:
    ...
    def _business_evaluation(self, insight_data: Dict, context: Dict = None) -> tuple:
        ...
        if business_impact == "critical" and roi_score < 0.3:
            ...
            return True, min(roi_score + 0.3, 1.0), f"Business impact {business_impact} with acceptable ROI"
    ...
    def _generate_supporting_data(self, insight_data: Dict) -> Dict:
        ...
        return {
            ...
            "confidence_factors": ["data_quality", "historical_patterns", "rule_compliance"]
        }
...
class MCPConsensusEngine:
    ...
    def _initialize_default_agents(self):
        ...
        logger.info(f"🤝 Initialized MCP consensus engine with {len(self.agents)} agents")
        
    def register_agent(self, agent_id: str, role: AgentRole, confidence_base: float = 0.8):
    ...
    async def request_consensus(self, request: MCPConsensusRequest) -> ConsensusResult:
        ...
        logger.info(f"🗳️ Starting consensus session {consensus_id} for insight type: {request.insight_type}")
        
        # Filter agents based on requirements
        participating_agents = self._select_participating_agents(request.required_agent_roles)
    ...
    def _select_participating_agents(self, required_roles: List[AgentRole]) -> List[ConsensusAgent]:
    ...
    async def _collect_votes(self, agents: List[ConsensusAgent], request: MCPConsensusRequest, 
                           consensus_id: str) -> List[ConsensusVote]:
        """Collect votes from all participating agents"""
        vote_tasks = []
        
        for agent in agents:
            self.debate_logs[consensus_id].append(f"Requesting vote from {agent.agent_id} ({agent.role.value})")
        ...
        try:
            ...
            self.debate_logs[consensus_id].append("TIMEOUT: Not all agents responded in time")
            raise
            
    def _analyze_consensus(self, consensus_id: str, votes: List[ConsensusVote], 
                          request: MCPConsensusRequest, start_time: float,
                          participating_agents: List[ConsensusAgent]) -> ConsensusResult:
        ...
        if not quorum_met:
            final_reasoning = f"Quorum not met: only {len(votes)}/{total_agents} agents participated"
    ...
    def get_consensus_statistics(self) -> Dict:
        ...
        approved_sessions = sum(1 for result in self.consensus_history if result.decision_approved)
        
        avg_consensus_level = sum(result.consensus_level_achieved for result in self.consensus_history) / total_sessions
        avg_processing_time = sum(result.processing_time_ms for result in self.consensus_history) / total_sessions
...
async def require_consensus(insight_type: str, insight_data: Dict, 
                          consensus_level: ConsensusLevel = ConsensusLevel.MAJORITY,
                          required_roles: List[AgentRole] = None) -> ConsensusResult:
    """
