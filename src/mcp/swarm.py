    ...
    def assess_task_fit(self, task: TaskDefinition) -> float:
        ...
        for requirement in task.requirements:
            for capability in self.capabilities:
                if (
                    requirement.lower() in capability.name.lower()
                    or requirement.lower() in capability.domains
                ):
...
class CollectiveMind:
    ...
    async def _broadcast_message(self, message: SwarmMessage):
        ...
        for agent in self.agents.values():
            if (
                agent.status == AgentStatus.ACTIVE
                and agent.agent_id != message.sender_id
            ):
                response = await agent.process_message(message)
                if response:
    ...
    async def propose_consensus(self, proposal: ConsensusProposal) -> str:
        ...
        logger.info(
            f"Consensus proposal {proposal.proposal_id} broadcast to collective"
        )
    ...
    async def distribute_task(self, task: TaskDefinition) -> str:
        ...
        if selected_agents:
            ...
            for agent_id, fit_score in selected_agents:
                await self.send_message(
                    SwarmMessage(
                        ...
                        content={"task": task.model_dump(), "fit_score": fit_score},
                        timestamp=datetime.now().isoformat(),
                        priority=3,
                    )
                )
            ...
            await self._activate_emergency_protocol(
                "no_suitable_agents",
                {"task_id": task.task_id, "requirements": task.requirements},
            )
    ...
    async def calculate_consciousness_cost(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost_per_decision": self.total_tokens_used
            / max(self.metrics["consensus_decisions"], 1),
    ...
    def get_collective_health(self) -> Dict[str, Any]:
        ...
        isolated_agents = [
            a for a in self.agents.values() if a.status == AgentStatus.ISOLATED
        ]

        avg_trust = sum(a.trust_score for a in active_agents) / max(
            len(active_agents), 1
        )
        ...
        return {
            ...
            "specializations_available": list(
                set(spec for agent in active_agents for spec in agent.specializations)
            ),
            "consciousness_cost": asyncio.create_task(
                self.calculate_consciousness_cost()
            ),
        }
        ...
        return {
            ...
            "autonomous_decisions_made": len(
                [
                    ...
                    if m.message_type
                    in ["task_assigned", "consensus_proposal", "specialist_created"]
                ]
            ),
            "collective_response": "Autonomous planning and specialization demonstrated",
            "emergent_behavior_detected": new_agents_created > 0
