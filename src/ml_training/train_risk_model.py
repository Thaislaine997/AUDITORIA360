...
def load_data_from_bq(project_id: str, dataset_id: str, table_name: str) -> pd.DataFrame:
    client = bigquery.Client(project=project_id)
    query = f"SELECT * FROM `{project_id}.{dataset_id}.{table_name}`"
    return client.query(query).to_dataframe()

def preprocess_data(df: pd.DataFrame, target_column: str) -> tuple[pd.DataFrame, pd.Series, list]:
...
def register_model_vertex_ai(model_display_name: str, model_artifact_uri: str, serving_container_image_uri: str):
...
if __name__ == "__main__":
    ...
    if not df_treinamento.empty:
        ...
        model_file, features_file = save_model_artifacts(model, features_usadas, local_path="meu_modelo_treinado")
