...
class CommunicationGateway:
        ...
        try:
            ...
            if provider_name:
                ...
                if not default_provider_name:
                    raise ValueError(
                        f"No default provider configured for {message_type}"
                    )
            ...
            if not provider.is_configured():
                raise ValueError(
                    f"Provider {provider.get_provider_name()} is not properly configured"
                )
    ...
    def get_provider_by_name(
        self, name: str
    ) -> Optional[CommunicationProviderInterface]:
        """
    ...
    def _get_supported_message_types(
        self, provider: CommunicationProviderInterface
    ) -> List[str]:
        """
    ...
    if twilio_config:
        ...
        if twilio_provider.is_configured():
            gateway.register_provider(
                twilio_provider, is_default_for=[MessageType.SMS, MessageType.WHATSAPP]
            )
