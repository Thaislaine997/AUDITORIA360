    ...
    def send_message(
        ...
        **kwargs,
        ...
        if message_type != MessageType.EMAIL:
            raise ValueError(
                f"EmailProvider only supports EMAIL messages, got {message_type}"
            )
...
class TwilioProvider(CommunicationProviderInterface):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.from_whatsapp = self.config.get(
            "from_whatsapp"
    ...
    def send_message(
        ...
        **kwargs,
        ...
        if message_type not in [MessageType.SMS, MessageType.WHATSAPP]:
            raise ValueError(
                f"TwilioProvider only supports SMS and WhatsApp, got {message_type}"
            )
    ...
    def process_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            clean_number = (
                from_number.replace("whatsapp:", "") if is_whatsapp else from_number
            )

            logger.info(
                f"Received {message_type.value} message from {clean_number}: {body}"
            )
            ...
            return {"provider": "twilio", "error": str(e), "status": "error"}
