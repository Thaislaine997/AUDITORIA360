...
class GitHubIntegrationService:
    ...
    def create_issue_from_message(
        self, message_data: Dict[str, Any], provider: str = "unknown"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Dados da Issue
            issue_data = {"title": title, "body": issue_body, "labels": labels}
            ...
            if response["success"]:
                logger.info(
                    f"GitHub Issue created successfully: {response['issue_url']}"
                )
                return {
                    "success": True,
...
"""
    ...
    def send_response_message(
        self, issue_number: int, response_message: str = None
    ) -> Dict[str, Any]:
        """
