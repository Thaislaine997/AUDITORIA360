...
class UserService:
    ...
    def create_super_admin_user(self) -> Dict[str, any]:
        ...
        """
        admin_password = self._get_secure_password(EnvironmentVariables.DEFAULT_ADMIN_PASSWORD)
    ...
    def create_test_users(self) -> List[Dict[str, any]]:
        ...
        gestor_a_password = self._get_secure_password(EnvironmentVariables.DEFAULT_GESTOR_A_PASSWORD)
        gestor_b_password = self._get_secure_password(EnvironmentVariables.DEFAULT_GESTOR_B_PASSWORD)
        client_x_password = self._get_secure_password(EnvironmentVariables.DEFAULT_CLIENT_X_PASSWORD)
    ...
    def validate_user_data(self, user_data: Dict[str, any]) -> Tuple[bool, Optional[str]]:
        ...
        """
        required_fields = ["username", "email", "full_name", "password_hash", "user_type"]
        ...
        # Validate user type
        valid_user_types = [UserTypes.SUPER_ADMIN, UserTypes.CONTABILIDADE, UserTypes.CLIENTE_FINAL]
        if user_data["user_type"] not in valid_user_types:
            return False, f"Tipo de usuário inválido: {user_data['user_type']}"
        
        return True, None
    
    def validate_company_data(self, company_data: Dict[str, any]) -> Tuple[bool, Optional[str]]:
        """
    ...
    def log_action(self, user_id: Optional[int], action: str, details: Dict[str, any] = None) -> Dict[str, any]:
        """
