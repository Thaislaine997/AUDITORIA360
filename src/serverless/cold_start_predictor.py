...
class ColdStartPredictor:
    ...
    def scan_codebase_functions(
        self, base_path: str = "/home/runner/work/AUDITORIA360/AUDITORIA360"
    ) -> List[Dict[str, Any]]:
        """
    ...
    def generate_training_data(
        self, function_analyses: List[Dict[str, Any]]
    ) -> pd.DataFrame:
        """
        ...
        for analysis in function_analyses:
            ...
            training_data.append({**analysis, "cold_start_ms": predicted_cold_start})
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if "module_type" not in self.label_encoders:
            ...
            X_df["module_type"] = self.label_encoders["module_type"].fit_transform(
                X_df["module_type"].fillna("other")
            )
        else:
            X_df["module_type"] = self.label_encoders["module_type"].transform(
                X_df["module_type"].fillna("other")
            )
    ...
    def predict_cold_start(self, function_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        features = [
            ...
            self.label_encoders["module_type"].transform(
                [function_analysis.get("module_type", "other")]
            )[0],
        ]
        ...
        return {
            ...
            "confidence": (
                "high"
                if predicted_cold_start < 500
                else "medium" if predicted_cold_start < 1500 else "low"
            ),
            "category": (
                "fast"
                if predicted_cold_start < 200
                else "medium" if predicted_cold_start < 800 else "slow"
            ),
        }

    def analyze_top_slow_functions(
        self, function_analyses: List[Dict[str, Any]], top_n: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        for analysis in function_analyses:
            ...
            try:
                ...
                print(
                    f"⚠️ Erro predizendo {analysis.get('relative_path', 'unknown')}: {e}"
                )
            ...
            top_slow_functions.append(
                {
                    ...
                    "potential_improvement_ms": self._estimate_potential_improvement(
                        func, optimization_suggestions
                    ),
                }
            )
        ...
        for func in top_slow_functions:
            print(
                f"   {func['rank']}. {func['file_path']}: {func['predicted_cold_start_ms']:.0f}ms"
            )
    ...
    def _generate_optimization_suggestions(
        self, function_analysis: Dict[str, Any]
    ) -> List[str]:
        ...
        if function_analysis.get("has_ml_dependencies"):
            suggestions.append(
                "Considere usar importação dinâmica para dependências ML"
            )
        ...
        if function_analysis.get("file_size_kb", 0) > 30:
            ...
            suggestions.append(
                "Extraia constantes e configurações para arquivos separados"
            )
    ...
    def _estimate_potential_improvement(
        self, function_analysis: Dict[str, Any], suggestions: List[str]
    ) -> float:
    ...
    def generate_analysis_report(
        ...
        top_slow_functions: List[Dict[str, Any]],
    ) -> str:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Funções com dependências ML**: {sum(1 for f in function_analyses if f.get('has_ml_dependencies'))}\n"
            )
            f.write(
                f"- **Dependência média**: {np.mean([f.get('dependency_size_mb', 0) for f in function_analyses]):.1f} MB\n"
            )
            f.write(
                f"- **Complexidade média**: {np.mean([f.get('complexity_score', 0) for f in function_analyses]):.1f}\n\n"
            )
            ...
            for func in top_slow_functions:
                ...
                f.write(
                    f"- **Cold start previsto**: {func['predicted_cold_start_ms']:.0f}ms\n"
                )
                ...
                f.write(
                    f"- **Melhoria potencial**: -{func['potential_improvement_ms']:.0f}ms\n\n"
                )
...
async def demo_cold_start_predictor():
    ...
    top_slow_functions = predictor.analyze_top_slow_functions(
        function_analyses, top_n=3
    )

    # 5. Gerar relatório
    print("\n🎯 FASE 5: Geração de Relatório")
    report_path = predictor.generate_analysis_report(
