...
class EtherealFunctionFireworks:
    ...
    async def ignite_function(
        self,
        function_type: str,
        request_data: Dict[str, Any],
        simulate_cold_start: bool = True,
    ) -> FireworkFunction:
        """
        ...
        firework = FireworkFunction(
            function_id=function_id, created_at=time.time(), request_data=request_data
        )
        ...
        with self._lock:
            ...
            self.metrics["concurrent_peak"] = max(
                self.metrics["concurrent_peak"], len(self.active_functions)
            )
    ...
    async def execute_function(self, firework: FireworkFunction) -> FireworkFunction:
        ...
        try:
            ...
            result = await self._execute_function_logic(
                function_type, firework.request_data
            )
    ...
    def _calculate_cold_start(self, function_type: str, request_data: Dict) -> float:
        ...
        complexity_factor = request_data.get(
            "complexity", 1
        ...
        cold_start_ms = (
            base_cold_start + data_size_factor + complexity_factor * 20
        ) * variation
    ...
    async def _execute_function_logic(
        self, function_type: str, request_data: Dict
    ) -> Dict[str, Any]:
    ...
    async def fireworks_storm_simulation(
        self,
        ...
        sustain_results = await self._execute_sustain_phase(target_rps, sustain_seconds)
        ...
        test_results.update(
            {
                ...
                "success_rate_percentage": (
                    len(self.completed_functions)
                    / max(test_results["requests_sent"], 1)
                )
                * 100,
        ...
        if self.completed_functions:
            avg_response_time = sum(
                f.execution_time_ms for f in self.completed_functions
            ) / len(self.completed_functions)
        ...
        print(
            f"   📈 Pico de concorrência: {test_results['peak_concurrent_functions']:,}"
        )
        ...
        print(
            f"   {'✅ TESTE APROVADO!' if test_results['test_passed'] else '❌ TESTE REPROVADO!'}"
        )

        return test_results

    async def _execute_ramp_up_phase(
        self, target_rps: int, duration_seconds: int, start_time: float
    ) -> Dict[str, Any]:
        ...
        for second in range(duration_seconds):
            ...
            if second % 2 == 0:  # A cada 2 segundos
                print(
                    f"   ⚡ {second+1}s: {current_rps} RPS ({len(self.active_functions)} funções ativas)"
                )
    ...
    async def _execute_sustain_phase(
        ...
        for second in range(duration_seconds):
            ...
            if second % 10 == 0:
                print(
                    f"   💥 {second+1}s: {target_rps} RPS ({len(self.active_functions)} funções ativas)"
                )
    ...
    async def _process_single_request(self, request_data: Dict[str, Any]):
        ...
        try:
            ...
            firework = await self.ignite_function(
                request_data["function_type"], request_data
            )
    ...
    def generate_fireworks_visualization(
        self, save_path: str = "/tmp/fireworks_visualization.png"
    ):
        """
        ...
        fig.suptitle(
            "🎆 Fogo de Artifício de Funções Serverless - AUDITORIA360",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i, (timestamp, event, func_type) in enumerate(
            zip(timestamps, events, function_types)
        ):
        ...
        if len(timestamps) > 0:
            ...
            for i in range(len(time_bins) - 1):
                count = sum(
                    1 for t in timestamps if time_bins[i] <= t < time_bins[i + 1]
                )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        execution_times = [
            f.execution_time_ms
            for f in self.completed_functions
            if f.execution_time_ms > 0
        ]
        cold_start_times = [
            f.cold_start_ms for f in self.completed_functions if f.cold_start_ms > 0
        ]

        return {
            "total_functions_executed": len(self.completed_functions),
            "total_functions_failed": sum(
                1 for f in self.completed_functions if f.status == "failed"
            ),
            "peak_concurrent_functions": self.metrics["concurrent_peak"],
            ...
            "cold_start_stats": {
                ...
                "percentage_with_cold_start": (
                    len(cold_start_times) / len(self.completed_functions)
                )
                * 100,
                "total_cold_starts": len(cold_start_times),
            },
            "throughput": {
                "functions_per_second": len(self.completed_functions)
                / max(self.metrics.get("total_execution_time_ms", 1) / 1000, 1)
            },
        }
...
async def demo_serverless_fireworks():
    ...
    for func_type in test_functions:
        request_data = {"function_type": func_type, "complexity": 2, "data_size": 1000}
        ...
        print(
            f"   ✨ {func_type}: {firework.execution_time_ms:.1f}ms (cold start: {firework.cold_start_ms:.1f}ms)"
        )
    ...
    print(
        f"   • Cold starts: {metrics['cold_start_stats']['percentage_with_cold_start']:.1f}%"
    )
    print(
        f"   • Throughput: {metrics['throughput']['functions_per_second']:.0f} funções/seg"
    )
