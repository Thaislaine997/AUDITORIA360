...
class TenantIsolationMiddleware:
    ...
    def apply_rls_filter(self, db: Session, tenant_scope: TenantScope):
        ...
        try:
            if tenant_scope.user_role != "administrador":
                ...
                logger.debug(
                    f"Applied RLS filter for empresa_id: {tenant_scope.empresa_id}"
                )
    ...
    def validate_tenant_access(
        ...
        required_permission = (
            f"{operation}:{requested_data.get('resource_type', 'data')}"
        )
        if (
            required_permission not in tenant_scope.permissions
            and tenant_scope.user_role != "administrador"
        ):
            logger.warning(
...
def validate_cross_tenant_access(
    user_tenant_scope: TenantScope, target_empresa_id: str, operation: str = "read"
) -> bool:
