...
class SecretsManager:
    ...
    def get_default_passwords(self) -> Dict[str, str]:
        ...
        for key, password in passwords.items():
            if not password or password.startswith("changeme"):
                if self.environment == "production":
                    raise ValueError(
                        f"Production environment requires secure {key} password"
                    )
                else:
                    # Generate secure password for development
                    passwords[key] = self.generate_secure_password()
                    logger.warning(
                        f"Generated secure password for {key} - change in production"
                    )
    ...
    def get_database_url(self) -> str:
        ...
        if "user:password@localhost" in db_url:
            if self.environment == "production":
                raise ValueError(
                    "Production environment requires secure database credentials"
                )
    ...
    def get_secret_key(self) -> str:
        ...
        if not secret_key:
            if self.environment == "production":
                ...
                logger.warning(
                    "Generated temporary secret key - set SECRET_KEY environment variable"
                )
