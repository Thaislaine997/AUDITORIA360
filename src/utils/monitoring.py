...
class SystemMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        self.metrics.set_gauge("system_memory_percent", memory.percent, {"type": "virtual"})
        self.metrics.set_gauge("system_memory_available_bytes", memory.available, {"type": "virtual"})
        self.metrics.set_gauge("system_memory_used_bytes", memory.used, {"type": "virtual"})
        ...
        self.metrics.set_gauge("system_network_bytes_sent", net_io.bytes_sent, {"direction": "sent"})
        self.metrics.set_gauge("system_network_bytes_recv", net_io.bytes_recv, {"direction": "received"})
        
        # Process metrics
        process = psutil.Process()
        self.metrics.set_gauge("process_memory_percent", process.memory_percent(), {"type": "current_process"})
        self.metrics.set_gauge("process_cpu_percent", process.cpu_percent(), {"type": "current_process"})
...
class HealthChecker:
    ...
    async def _run_check(self, check: Dict[str, Any]) -> HealthCheck:
        ...
        try:
            ...
            # Record metrics
            self.metrics.set_gauge(f"health_check_response_time_ms", 
                                 response_time, {"check_name": check['name']})
            self.metrics.set_gauge(f"health_check_status",
                                 1 if status == 'healthy' else 0, {"check_name": check['name']})
...
class MonitoringSystem:
    ...
    def _setup_default_alerts(self):
        ...
        self.alert_manager.add_alert_rule(
            "system_cpu_percent", 80, "gt", AlertSeverity.HIGH,
            "High CPU Usage", "CPU usage is above 80%"
        )
        
        # Memory alerts
        self.alert_manager.add_alert_rule(
            "system_memory_percent", 85, "gt", AlertSeverity.HIGH,
            "High Memory Usage", "Memory usage is above 85%"
        )
        
        # Disk alerts
        self.alert_manager.add_alert_rule(
            "system_disk_percent", 90, "gt", AlertSeverity.CRITICAL,
            "High Disk Usage", "Disk usage is above 90%"
        )
