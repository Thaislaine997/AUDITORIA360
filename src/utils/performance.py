...
class PerformanceProfiler:
    ...
    def profile_function(self, include_params: bool = False):
        ...
        def decorator(func: Callable):
            ...
            async def async_wrapper(*args, **kwargs):
                return await self._profile_async_execution(func, args, kwargs, include_params)
            
            return async_wrapper if asyncio.iscoroutinefunction(func) else wrapper
        return decorator
    
    def _profile_execution(self, func: Callable, args: tuple, kwargs: dict, include_params: bool):
        ...
        try:
            ...
            metrics = PerformanceMetrics(
                ...
                parameters={"args_count": len(args), "kwargs_keys": list(kwargs.keys())} if include_params else None,
    ...
    async def _profile_async_execution(self, func: Callable, args: tuple, kwargs: dict, include_params: bool):
    ...
    def _record_metrics(self, metrics: PerformanceMetrics):
        ...
        with self._lock:
            ...
            if metrics.execution_time > self.slow_queries_threshold:
                logger.warning(f"Slow operation detected: {metrics.function_name} took {metrics.execution_time:.2f}s")
            
            # Log high memory usage
            if metrics.memory_usage_mb > self.memory_threshold:
                logger.warning(f"High memory usage: {metrics.function_name} used {metrics.memory_usage_mb:.2f}MB")
    ...
    def get_bottlenecks(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        for func_name, stats in function_stats.items():
            ...
            # 2. High memory usage
            # 3. Error rate > 5%
            if (stats['avg_time'] > self.slow_queries_threshold or 
                stats['avg_memory'] > self.memory_threshold or
                (stats['error_count'] / stats['total_calls']) > 0.05):
    ...
    def _get_recommendations(self, func_name: str, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats['avg_memory'] > self.memory_threshold:
            recommendations.append("Optimize memory usage - consider streaming or pagination")
            recommendations.append("Review data structures and remove unnecessary object creation")
...
class DatabaseOptimizer:
    ...
    def optimize_postgresql_query(self, query: str) -> Dict[str, Any]:
        ...
        if 'where' in query_lower and 'index' not in query_lower:
            optimizations['suggestions'].append("Consider adding indexes on WHERE clause columns")
            optimizations['estimated_improvement'] += 30
        
        # Check for SELECT *
        if 'select *' in query_lower:
            optimizations['suggestions'].append("Avoid SELECT * - specify only needed columns")
            optimizations['estimated_improvement'] += 20
        
        # Check for JOIN optimization
        if 'join' in query_lower and 'on' in query_lower:
            optimizations['suggestions'].append("Ensure JOIN conditions use indexed columns")
            optimizations['estimated_improvement'] += 25
        
        # Check for ORDER BY without LIMIT
        if 'order by' in query_lower and 'limit' not in query_lower:
            optimizations['suggestions'].append("Consider adding LIMIT to ORDER BY queries")
    ...
    def optimize_duckdb_query(self, query: str) -> Dict[str, Any]:
        ...
        if 'csv' in query_lower:
            optimizations['suggestions'].append("Consider using Parquet format for better performance")
            optimizations['estimated_improvement'] += 40
        
        # Check for column projection
        if 'select *' in query_lower:
            optimizations['suggestions'].append("Project only necessary columns for better I/O performance")
            optimizations['estimated_improvement'] += 25
        
        # Check for filter pushdown
        if 'where' in query_lower:
            optimizations['suggestions'].append("Ensure filters are applied early (filter pushdown)")
            optimizations['estimated_improvement'] += 20
